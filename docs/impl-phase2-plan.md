...
1. **Отмена выполнения не откручивает лесенку.** `uncomplete` возвращает задачу в `pending` и
   ...
   состояния «до» нигде не хранится, откатывать нечего. **Решение (Elmir 2026-08-01): реверс нужен
   — отменил отметку, значит её не было.** Как — разбирается в 2.7.3 (нужен снимок «до» на строке
   задачи + правило «откатываем только последнюю двигавшую лесенку отметку»).
