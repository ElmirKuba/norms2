...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  («Шаблоны»-список), `2.4·16a` ✅ (модалка привычки create/edit — разбили ·16 на ·16a/·16b).
  Следующее — `2.4·16b` (сфера + атрибуты в модалке).**
...
- [ ] **2.4·16 (фронт: модалка привычки) — РАЗБИТ на ·16a/·16b** (Elmir: тяжёлая форма, де-риск).
  ...
  - [ ] **·16b (сфера + атрибуты)** — в модалку: select **сферы** (домены) + мультиселект
    **RPG-атрибутов** (каталоги через api ·16a). **Получим:** привязка привычки к сфере/атрибутам.
