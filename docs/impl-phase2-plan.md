...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·13` (привычка→цель, кросс-домен ВНИЗ) — ЗАКРЫТ ✅ LIVE (смоук 9/9):** complete привычки докидывает
  ...
  ГОТОВ И LIVE-VERIFIED** (миграции до **0015**, dev чистая accounts=18). **ФРОНТ ЦЕЛЕЙ В РАБОТЕ:** `2.5·14` (типы+api) — ЗАКРЫТ ✅ (tsc app).
  **СЛЕДУЮЩЕЕ — `·15` список целей `/accent/goals` (заменит мёртвую заглушку-вкладку, закроет P2.3): карточки
  с %/forecast/бейдж рода, пустой экран, фильтры status/domain → `·16` модалка создания/редактирования →
  `·17` карточка цели (запись прогресса + визуализация %/forecast/история) → `·18` вехи → `·19` подцели →
  `·20` привязка привычка↔цель в форме привычки → `·21` полировка → деплой/ретро `·22–·23`.** Нарезка ·15–·23 ниже.
