...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Код начат. 2.0.0·1 ✅ (модуль accent, boot чист). Следующее — `2.0.0·2` (БД
  `accent_settings` + Drizzle-схема + миграция).** На прод НЕ деплоим до 2.0.0·6
  (каркас невидим, пока нет пункта меню/навигации — реш. Elmir).
