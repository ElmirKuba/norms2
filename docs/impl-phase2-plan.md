...
- [ ] **·4 (бэк+фронт)** — публичные `/releases` и `/releases/:key`: контроллер **без Guard**, только релизные ноты, **без отметок о прочтении**; экран в публичной зоне, рендер тем же `renderMarkdown` и глобальными стилями `.md`. Текст ноты и так публичен (`content/*.md` без токена) — закрыт был только API центра уведомлений.
