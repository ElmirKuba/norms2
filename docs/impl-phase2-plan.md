...
> **Порядок работ 2.6.1/2.6.2 (методология: сперва дока+ADR, потом код, инкрементально с отметками в плане):** (1) ✅ **ADR написаны** — [ADR-0059](./decisions/0059-anti-habit-timeline-events.md) (модель событий `anti_habit_events`) + [ADR-0060](./decisions/0060-anti-habit-calendar-goal-ladder.md) (календарная лестница целей); (2) синхрон `domain-model §7` + `api-contracts §7` ← **следующий шаг**; (3) код D4→D5→D6→(D2 схема→домен→API→фронт)→(D3)→D7→D1; (4) feature-file; (5) live-verify; (6) бандл-деплой. Детальную нарезку по шагам — при старте каждого блока.
