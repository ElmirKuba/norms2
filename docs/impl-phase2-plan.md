...
- **Следующее:** `gamification.md` (правила: очки/лесенка/состояния) — там же решаем
  открытые R8 (серии) / R11 (формула XP).
- **Терминология (#1) подтверждена**; маппинг к ТЕКСТУ доки (`domain-model`/`ui-ux`)
  применяем при финализации файла (пока зафиксирован в журнале).
- **Открыто:** слить ли `CheckIn`/`DailyMetric` (предложено слить — см. развилки).
...
- 💬 3. CheckIn и модель состояний (StateResolver — старт простой #3; merge с DailyMetric — открыто)
...
- 💬 10. Недельный слой и журналы (DailyMetric ↔ CheckIn merge — открыто)
...
- **CheckIn ↔ DailyMetric (слияние)** — оба пишут дневное самочувствие (mood/energy/
  sleep), дублируются. Предложено слить в единый дневной снимок (CheckIn), DailyMetric
  убрать, тренды строить из CheckIn. Elmir просит подробно объяснить → решаем после.
...
- Форк **CheckIn↔DailyMetric** — Elmir просит объяснение → в работе (см. развилки).
