...
- [ ] **·1 (docs)** — контракт `GET /accent/habits/:id/history` в [`api-contracts`](./sections/accent/api-contracts.md): форма ответа, keyset-пагинация, явно «эндпоинт **ничего не создаёт**, `ensureTasksForDay` не зовёт». Роут `habits/:id` — в карту экранов [`ui-ux §4`](./sections/accent/ui-ux.md).
