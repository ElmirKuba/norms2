...
- Репозитории — слой `repositories`. Схемы — `system/orm-schemas`, связи — `system/orm-relations`.
...
Слои папками в корне `src`. Порядок СВЕРХУ ВНИЗ (как в kuba-game), поток `controller → use-case → domain-service → adapter → repository`:

1. **`controllers`** — контроллеры: HTTP, валидация входа (DTO), вызов **use-case**.
2. **`use-cases`** — верхнеуровневая оркестрация сценария. **Точка кросс-доменных вызовов.** Зовёт domain-service(ы) своей и/или других областей.
3. **`domain-services`** — бизнес-логика одной доменной области; зовёт adapter своей области.
4. **`adapters`** — граница домен↔инфраструктура (порт-подобный слой); зовёт репозитории.
5. **`repositories`** — доступ к данным через Drizzle.
...
Кросс-доменные зависимости идут **только вниз**: `use-case` области A может звать **`domain-service` другой** области B (слой ниже), а НЕ её use-case. `domain-service` не зависит от `use-case` → цикла NestJS-DI нет.
Пример: `account.use-case → courses.domain-service` И `courses.use-case → account.domain-service` — оба валидны, цикла нет. В 4-слойке (norms) такого разведения не было, отсюда риск `forwardRef`/циклов; 5-слойка снимает это структурно и делает систему расширяемой.
(Связывание модулей: каждый модуль слоя `imports` модуль слоя ниже и `exports` свой сервис; пример из kuba-game: `*.domain-service.module → imports AdapterModule`, `*.use-case.module → imports DomainServiceModule(ы)`, controller-module → imports UseCaseModule.)
(Историческая заметка: в исходном kuba-game слои назывались `api-endpoints / use-cases-level / managers-level / drizzle-repositories`; в Нормисах приняты канон-имена `controllers / use-cases / domain-services / adapters / repositories`.)
