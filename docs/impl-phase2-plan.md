...
- **Висит:** финальное подтверждение маппинга терминов (#1, таблица в журнале).
...
- **#4 (структура, ПРИНЯТО):** фронт — «Акцент» = пункт верхнего меню (рядом с
  «Приглашения») = мини-приложение со вложенной навигацией; бэк — набор под-модулей
  под `accent/` (goals/habits/gamification/state/…).
- **#1 (терминология, направление ПРИНЯТО — маппинг на подтверждении):** русифицируем/
  нейтрализуем (выкинуть Epic/Secret/BadGuy). Код — нейтрально-англ., UI — рус.:
