...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Код: ·1 ✅ (зонтик AccentModule), ·2 ✅ (таблица accent_settings, миграция 0003 на dev),
  ·3 ✅ (AccentSettingsModule: порт+репо+domain-service, boot чист). Следующее — `2.0.0·4`
  (API: use-cases + контроллер `GET/PATCH /accent/settings`, `POST /accent/pause|resume`
  под AuthGuard — импорт `AccessControlModule` + zod-DTO + синхрон api-contracts §1).**
  На прод НЕ деплоим до 2.0.0·6; прод-миграция 0003 — при том деплое.
