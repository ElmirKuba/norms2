...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Следующее — подфаза `2.1` (справочники: `accent_domains`/`accent_attributes` + сид + API).**
...
- [ ] **2.1·1 (БД)** — схемы `accent_domains` + `accent_attributes`: **PK = `key`**
  (slug `health`/`strength`…, varchar — не uuid; прецедент: `Achievement.code` в ф.1),
  + `title`, + `position` (порядок), + `is_active`/`icon?` (опц.). Интерфейсы ADR-0033 +
  миграция (накатить dev). _Нюанс ADR-0016: для каталогов-справочников PK = slug, не uuidv7._
- [ ] **2.1·2 (бэк)** — `AccentReferenceModule`: порт + Drizzle-репо
  (`listDomains()`/`listAttributes()` — активные, по `position`) + (тонкий) domain-service.
- [ ] **2.1·3 (API)** — контроллер `GET /accent/domains`, `GET /accent/attributes` (AuthGuard);
  use-cases (тонкие); зонтик `AccentModule` импортит `AccentReferenceModule`. Синхрон api §1a.
- [ ] **2.1·4 (сид)** — идемпотентный сид дефолтных сфер (health/sleep/sport/work/money/
  relationships/learning/home/creativity/purpose) и атрибутов (strength/discipline/intellect/
  spirit/social/health) на `OnApplicationBootstrap` (ON CONFLICT по `key`). Проверить live.
- **Деплой 2.1?** — НЕВИДИМ для юзера (нет UI-потребителя) → **открытый вопрос:** силент-деплой
  без бампа VERSION/broadcast, или бандлить в следующий видимый релиз (2.2). Решить при выкате.

### 2.2–2.11 — скелет (детальные шаги — при старте подфазы)
