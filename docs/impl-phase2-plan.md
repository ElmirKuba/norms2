...
- [ ] **·5 (бэк, вехи «держусь»)** — 3/7/14/30/60/90 дней текущей попытки, **проверка при чтении** (как ленивая материализация дня), без `@nestjs/schedule`. Накопилось несколько — **показываем только старшую**. Здесь же снимаются два TODO-маркера в `logging-anti-habit-events.adapter.ts`. Очков за веху нет: веха — факт, о котором сообщили.
