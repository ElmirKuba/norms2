...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  `2.2·5` ✅ (стартовый набор), `2.2·6` ✅ (фронт `/accent/micro-wins`: список, complete одним
  тапом, CRUD-модалка; tsc+prod-сборка strictTemplates чистые). Следующее — **`2.2·7` (деплой
  2.2.0 = бандл 2.1+2.2: VERSION→2.2.0, release-2.2.0.md+broadcast, prod-build/up/migrate, smoke)**.**
