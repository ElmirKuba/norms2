...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Текущая прод-версия — 2.5.1.** Следующее по плану раздела — подфаза `2.6` (AntiHabit «держусь»).
...
- [~] **BUG-1 🔴 Вход на новом устройстве выкидывает другое (кросс-девайс «кик»).** Симптом: логин Chrome@iPhone → кикает Chrome@MacBook, и наоборот. **ФИКС В КОДЕ ГОТОВ (2.5.2, коммит 908e610 + ADR-0046 ревизия 2026-07-01); live 2-девайс repro — за Elmir перед деплоем.**
  ...
  - **Решение (ADR-уровень — согласовано):** (б) бэк — смягчить reuse-detect: **грейс-окно 15с** на конкурентную ротацию тем же токеном + отзыв **только реплейнутой сессии**, не всех (ревизия [ADR-0046](./decisions/0046-refresh-reuse-detection-and-recovery-posture.md)). Это **истинный фикс**: нейтрализует benign-гонку независимо от фронта. (а) фронт single-flight refresh в интерцепторе **уже есть** (`auth.interceptor.ts` `refresh$`+`shareReplay`); `sessionInitializer` шлёт свой refresh до старта роутинга (не конкурирует внутри вкладки) → доп. правки фронта не требуется под этот симптом.
  - **✅ СДЕЛАНО В КОДЕ (2026-07-01):** `SessionDomainService.rotateSession` — грейс `REUSE_GRACE_MS=15_000`: архивный токен в пределах грейса → мягкий 401 без отзыва; вне грейса → `revokeById` только той сессии; проигрыш CAS → мягкий 401. `revokeAllByAccount` в reuse-detect **убран** (grep: остались лишь recovery/delete/deactivate). Контроллер на 401 refresh cookie не чистит (не затирать токен победителя). nest tsc зелёный. Аудит границ пройден.
  - **⏳ ОСТАЁТСЯ — live-verify (Elmir, перед деплоем 2.5.2).** Рецепт repro на стенде/прод-деве: 1) залогиниться на устройстве A и B (одним аккаунтом); 2) на A нажать refresh/перезагрузить пару раз (провоцируем двойной `/auth/refresh`); 3) убедиться, что сессия B **жива** (запрос с B → 200, не 401); 4) в БД: `SELECT id, revoked_at, created_at FROM sessions WHERE account_id=... ORDER BY created_at DESC` — **нет парных revoke в одну секунду** при штатном refresh; `session_token_history` растёт по строке на ротацию. 5) Негатив-тест кражи: сохранить старый refresh-токен, подождать >15с, предъявить → ожидаем 401 + revoked **только** этой сессии (остальные живы).
- [ ] **BUG-2 🟠 Нет старт-даты привычки → нельзя «через день»/чередовать.** Симптом: «каждые N дней» N=2 — хочу две привычки в противофазе, но обе от сегодня попадают на одни дни; вторую пришлось бы заводить завтра.
  - **Анализ — ПОДТВЕРЖДЕНО:** якорь RRULE = дата создания (`dtstart = localYmd(habit.createdAt, timezone)` в `accent-task.domain-service.ensureTasksForDay`). Утилита якорь уже принимает (`isHabitDueOn(recurrence, dtstart, date)`), но в схеме `habits` **нет пользовательского поля старт-даты** → фаза INTERVAL-правил жёстко на `created_at`.
  - **Решение (мелкая подфаза `2.x` / вместе с 2.6):** колонка `habits.start_date` (date, nullable, default = дата создания в TZ) → миграция; пробросить в `dtstart` вместо `created_at`; DTO `startDate?` (create/update); UI «Начать с» в форме привычки (дефолт сегодня, можно завтра). Чередование = сдвиг старт-даты. Бэкфилл не нужен (null → fallback на `created_at`).
