...
>
> Бизнес-слои (managers/use-cases) **не импортируют ORM** напрямую — доступ к данным через `adapters` → `repositories` (5-слойка, [ADR-0030](./decisions/0030-stack-revision-drizzle-5layer-npm.md)). ([CLAUDE.md](../CLAUDE.md))
