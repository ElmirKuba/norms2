...
**Блок J — закрытие**
- [x] **2.7·38** (частично) ✅ 2026-07-31 — **браузер-verify 13/14** (Playwright, dev, аккаунт `elmir_kuba`): экран открылся, 18 карточек, бейджи «пример», «Добавить себе» снял бейдж (15→14), модалка «Столкнулся» со своими ответами, «Просто отметить» обновило счётчик **без перезагрузки**, деталь (контрмеры сверху + «О препятствии» + лента), ссылка «К препятствиям» ведёт в раздел, кнопка «🌙 Минимум сегодня: 1 отжимание» на карточке дня. 14-й пункт — «ноль ошибок консоли» — упирается в штатный `401 POST /auth/refresh` до входа (то же было в verify 2.6), к разделу отношения не имеет. **Паспорт [`feature-obstacles.md`](./feature-obstacles.md) переписан по факту** + вывод сравнения: **продукт угадали, сложность — нет** (все 7 сценариев дожили без изменений; разошлись только «служит минимумом для…» — не сделано, порядок работ внутри блока и стоимость связи с «Держусь»). **Релиз-нота** `release-2.7.0.md` написана и зарегистрирована в сиде; **`VERSION` → `2.7.0`**. tsc чист.
