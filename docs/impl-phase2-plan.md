...
- [x] **2.5·21 (фронт) — ЗАКРЫТ ✅ (prod-сборка)** — фильтр по сфере в списке целей (select из каталога
  ...
  ЦЕЛЕЙ ·14–·21 ГОТОВ** (prod-сборки зелёные). Осталось: деплой `·22` + ретро `·23`.

**Деплой + ретро (2 шага):**
- [ ] **2.5·22 (деплой)** — `VERSION`-бамп, `release-*.md` + broadcast, `prod-build/up` (migrate-гейт), smoke.
  **ОТЛОЖЕН (реш. Elmir 2026-06-23):** код 2.5 полностью готов и запушен, но на прод НЕ выкатываем пока;
  прод остаётся на 2.4.20. Деплой — по отдельному решению.
- [x] **2.5·23 (ретро 2.5 в 3 шляпы) — ЗАКРЫТ ✅** → [docs/retro-phase2-2.5.md](./retro-phase2-2.5.md).
  Архитектура чистая (grep-аудит), весь бэк live-verified, весь фронт prod-сборками.
  **+ FIX-ПРОХОД (Elmir «фиксим все», 2026-06-23):** исправлено **5/8** находок (P1 fallbackVersion виден ·
  P2#2 uncomplete-откат прогресса цели, миграция **0016**, live-смоук 4/4 · P3#6 reduce-тон · P3#7 rollup/
  записи не смешиваются · P3#8 forecast-хинт); **3/8 осознанно отложены** с обоснованием (P2#3 N+1 —
  преждевременная оптимизация для альфы; P2#4 дашборд — это подфаза 2.11; P3#5 children-endpoint — клиентский
  фильтр терпим). Таблица «Резолюция находок» — в retro-doc. Миграции до **0016**.
