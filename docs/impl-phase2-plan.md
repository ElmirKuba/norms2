...
- [ ] **2.5·23 (ТРИАЖ «цели не совсем работают») — В РАБОТЕ.** Elmir потыкал в браузере — запись замера/
  вехи ничего не меняли. Полный анализ кодовой базы целей. **Реестр находок + план — отдельный резюмируемый
  файл [docs/triage-2.5-goals.md](./triage-2.5-goals.md)** (методология Elmir: анализ-подпункты `·23.A*`
  находят баги → пишут в реестр → рождают фикс-подпункты `·23.F*`). Под-структура:
  - [x] **·23.A0 / F#1 — 🔴→✅ корневой баг найден и исправлен.** Формы «Записать» и «Добавить веху» в
    `goal-detail` были `<form (ngSubmit)>` **без `[formGroup]`** → `ngSubmit` не эмитился → POST не уходил
    (в логах nest 0 POST на entries/milestones; `milestones` пуста). Фикс: обе формы в `FormGroup`+`[formGroup]`+
    `formControlName` (как логин/модалки). Prod-build зелёный; **нужен браузер-смоук Elmir'а**.
  - [ ] **·23.A1** фронт goal-detail (swallowed-ошибки, обновление сигналов, история, forecast).
  - [ ] **·23.A2** фронт goals-list + dashboard + формы создания/правки.
  - [ ] **·23.A3** бэк: вычисление прогресса (ADR-0052) — 3 рода, edge, rollup, TZ, авто-complete.
  - [ ] **·23.A4** бэк: записи/вехи/lifecycle/привычка→цель (идемпотентность, права, валидации).
  - [ ] **·23.A5** границы слоёв (grep-аудит ORM/инфра не течёт в домен).
  - [ ] **·23.A6** свод + фикс-подпункты `·23.F*` + перепроверка. Только зелёное → выход к ·24.
  **DoD ·23:** реестр находок закрыт (всё 🔴/🟠 исправлено), браузер-смоук ключевых потоков зелёный.

- [ ] **2.5·24 (ФОКУС целей, anti-sprawl — combo ①A+C) + ADR.** Поле `goals.focus_order` (int, null):
  `null` = активна-но-не-в-фокусе, не-`null` = в фокусе + ранг. Даёт И мягкий фокус (предупреждение при
  >N, env-дефолт 3, **без блока**), И приоритет/порядок внутри фокуса. Список: секция «В фокусе» (сорт по
  `focus_order`) сверху, остальные активные ниже. Мягкое «уже N в фокусе — что-то отпустишь?» (не запрет).
  **Сначала ADR** (развилка Elmir: фокус-механизм; обратимость: флаг→инвариант позже). Бэк (колонка+миграция+
  toggle-эндпоинт+сорт) → фронт. Шляпы: **бизнес+помощь+миссия** (тренирует выбор, ADR-0049).
