...
- **Закрытие 2.6.3:** [x] `VERSION`→`2.6.3`; [x] tsc (nest+ng) + `ng build prod` зелёные; [x] **браузер-verify Playwright dev ✅** (валидный код → поздравление; переход → `/register?code=` + авто-прыжок к форме; невалидный → «не найдено»; модалка шэринга: ссылка + «Скопировать»/«Открыть»; скриншоты сверены). [ ] коммит/пуш/деплой (ждёт «го»).
