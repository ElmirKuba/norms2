...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·13` (привычка→цель, кросс-домен ВНИЗ) — ЗАКРЫТ ✅ LIVE (смоук 9/9):** complete привычки докидывает
  ...
  привязка привычка↔цель, полировка (фильтр сферы+hscroll). **🎉 ПОДФАЗА 2.5 «ЦЕЛИ» ПОЛНОСТЬЮ ГОТОВА В КОДЕ
  (бэк ·1–·13 + фронт ·14–·21), всё запушено.** Ретро `·23` ✅ ([retro-phase2-2.5.md](./retro-phase2-2.5.md);
  топ-находка P1 — показать `fallbackVersion` на детали). **Деплой `·22` ОТЛОЖЕН (реш. Elmir 2026-06-23):
  прод остаётся на 2.4.20, выкат 2.5.0 — по отдельному решению.** Браузер-клик целей визуально — за Elmir
  (бэк весь live-verified, фронт — prod-сборками). Следующее по плану раздела — подфаза `2.6` (AntiHabit «держусь»).
...
- [x] **2.5·23 (ТРИАЖ «цели не совсем работают») — ЗАКРЫТ ✅ (2026-06-24).** Полный анализ кодовой базы
  целей. **Реестр находок — [docs/triage-2.5-goals.md](./triage-2.5-goals.md)** (методология Elmir:
  анализ-подпункты `·23.A*` находят баги → реестр → фиксы). **Итог: 9 находок, все разобраны:**
...
- **2.5·24 (ФОКУС целей, anti-sprawl — combo ①A+C).** Поле `goals.focus_order` (int, null): `null` =
  ...
  **Разбит на резюмируемые под-шаги (выжить между лимитами):**
  - [ ] **·24.1 — ADR + env.** Написать `docs/decisions/00XX-goal-focus-and-mission-filter.md` (фиксирует
    A+C, N, env; общий с ·25 mission-filter). Добавить `ACCENT_GOAL_FOCUS_SOFT_LIMIT` (`z.coerce.number()
    .int().positive().default(3)`) в `nest/src/system/config/env.schema.ts` (образец — `ACCENT_GOAL_MAX_DEPTH`).
    **Заодно гигиена env:** задокументировать `ACCENT_GOAL_MAX_DEPTH` + новый комментариями в `.env.example`
    и `.env.prod.example` (сейчас тюнеры есть только в Zod-схеме, в примерах их нет). DoD: ADR закоммичен,
    env согласованы, tsc зелёный.
  - [ ] **·24.2 — БД + схема + интерфейсы.** Миграция: `goals.focus_order` (integer, null). `goals.schema.ts`
    +`focusOrder`; `GoalFull`/`GoalView` +`focusOrder`; маппер `toGoalView`. DoD: миграция применена на dev,
    tsc зелёный.
  - [ ] **·24.3 — порт + репозиторий.** Порт: `setFocus(id, accountId, order)` / `clearFocus(id, accountId)` /
    `countFocused(accountId)` / `maxFocusOrder(accountId)` (для следующего ранга). Drizzle-реализация в
    `database/`. DoD: tsc зелёный.
  - [ ] **·24.4 — домен + use-case + контроллер + DTO.** Домен `toggleFocus`: добавить в фокус → ранг =
    `maxFocusOrder+1`, вернуть `{focusedCount, softLimit, overLimit}` (НЕ блок); убрать → `clearFocus`.
    Эндпоинт `POST/DELETE /accent/goals/:id/focus` (объявить ДО `:id`-конфликтов). `list` отдаёт `focusOrder`.
    DoD: tsc зелёный + **live-смоук curl** (toggle, focusedCount, overLimit-флаг при >3).
  - [ ] **·24.5 — фронт: тип + API + список.** `GoalView` +`focusOrder`; `accent-api` `toggleGoalFocus`;
    `goals.component`: секция «⭐ В фокусе» (сорт по `focusOrder`) сверху, кнопка-toggle (звезда) на карточке,
    мягкая модалка «уже N в фокусе — что-то отпустишь? [Всё равно] [Отмена]» при `overLimit` (через
    `ModalService`, реактивно без F5). DoD: prod-build зелёный + браузер-смоук Elmir.
  - [ ] **·24.6 — (опц.) goal-detail toggle + дашборд.** Кнопка «в фокус/из фокуса» в меню «⋯» детали;
    дашборд «Цели в работе» показывает фокусные первыми. DoD: prod-build.
  - [ ] **·24.7 — верификация + свод.** Полный live-смоук, синхронизировать ADR-статус/доку/трекер с фактом.
    DoD: всё зелёное → **·24 закрыт**, переход к ·25.
...
- [ ] **2.5·25 (MISSION-FILTER для accumulate — combo ②A+B) + ADR (общий с ·24).** Дефолт — мягкая
  подсказка при создании accumulate-цели («это про важное — или “делать больше”?», рядом с «зачем», **не
  блок**). Плюс условно-обязательное поле «ради чего откажусь» (`tradeoff`) **только при переводе accumulate-
  цели В ФОКУС** (цена слота фокуса — связывает с ·24). Только `direction='accumulate'`; reduce/reach уже в
  миссии. Фронт-копирайт + (для tradeoff) поле/валидация. Шляпа: **помощь+миссия** (снимает «беговую дорожку»).

- [ ] **2.5·26 (`maintain` — 4-й род «удерживать уровень» — combo ③, отдельный выверенный шаг) +
  расширение ADR-0052.** Новое `direction='maintain'` (вес в коридоре, экран ≤X, сахар ≤X). Семантика
  иная: успех = **доля дней в коридоре за окно**, forecast иной, **НЕТ авто-completed** (длящаяся цель).
  Домен+формулы+edge (пустое окно, нарушение коридора, статус без завершения, валидация коридора). Самый
  инженерный — отдельным заходом, live-verify все ветки. Шляпа: **система 9→10**.

- [ ] **2.5·27 (DRAG-TO-REORDER трекеров — UX, ДО деплоя) + ADR.** Реш. Elmir (2026-06-24): «надо до
  FINISH, это улучшает опыт пользователя». Ручная сортировка перетаскиванием для **всех трёх трекеров**
  (микро-победы/привычки/цели): утащил карточку (была 3-я → вниз) — порядок запомнился. **Сервер-персист:
  колонка `position int` per-account в `micro_wins`/`habits`/`goals` → cross-device** (с другого устройства
  карточка на том же месте; `localStorage` НЕЛЬЗЯ). Нужны: миграции (`position`) + reorder-эндпоинт(ы)
  (обновление позиций, идемпотентно) + drag-UI + сортировка листингов по `position`. **Развилка (ADR, слово
  Elmir): Angular CDK DragDrop vs свой drag на нативных Pointer Events** («чистый SCSS, без китов»). Кусок
  drag-ранга фокуса целей (`focus_order`) смыкается с ·24. Шляпа: помощь/UX. Связь [[spa-no-reload-reactivity]].
  - **Порядок по умолчанию (реш. Elmir 2026-06-24):** `position` инициализируется так, чтобы **сохранить
    нынешнюю сортировку** (как сортируем сейчас — напр. по `created_at`): миграция-бэкфилл проставляет
    `position` в текущем порядке, новые элементы добавляются в конец (или начало — уточнить при постройке).
    Визуально до первого drag ничего не меняется; как только юзер перетащил — его ручной порядок берёт верх.
    Листинги сортируются по `position` (с детерминированным tie-break, напр. `id`).

- [ ] **2.5·28 (ГЛУБОКИЙ БАГХАНТ + внимательный анализ перед деплоем, был `·27`).** После ·23–·27 —
  тщательный разбор ВСЕЙ фичи целей: грубые потоки + edge + границы. Включить: grep-аудит чистоты слоёв
  (ADR-0030/0034/0050), все 3+1 родов, конкурентность записей (computed → гонок быть не должно, ADR-0052),
  TZ-границы дня, rollup смешанных единиц, фокус-лимит, mission-filter, стартер-пак, **drag/reorder+position**.
  **DoD:** письменный отчёт-анализ; 0 известных блокеров. Только зелёный ·28 открывает FINISH.

- [ ] **2.5·FINISH (деплой 2.5, был `·22`).** `VERSION`-бамп → `2.5.0`, `release-2.5.0.md` + broadcast,
  `prod-build/up` (migrate-гейт, миграции до текущей), prod-smoke. Прод уезжает с **2.4.20 → 2.5.0**.
  **Только после зелёного ·28.** (Ранее ОТЛОЖЕН реш. Elmir 2026-06-23 — теперь это финальный шаг подэтапа,
  а не «индивидуальный отложенный».)
