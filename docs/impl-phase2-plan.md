...
- **Нарезка (сперва дока, потом фронт; бэкенд не трогаем) — resumable подпункты:**
  ...
  - **H1·3 (verify):** ✅ 2026-07-06 — live-прогон через реальный UI (Playwright на dev-стеке):
    ...
    - **⚠️ Наблюдение:** «Готово раньше» логирует полную длительность (`doneValue=target`), а не фактически прошедшие секунды — приемлемо для MVP, но расходится с формулировкой «по факту» в H1.1b; кандидат на доработку (возвращать elapsed из модалки).
