...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  Следующее — подфаза `2.4` (привычки + задачи + адаптивная лесенка) — **нарезана мелко на ·1–·17**
  (детальная секция ниже). Решения: расписание = RRULE (`rrule`), ролловер = ленивый сейчас
  (фоновый cron `@nestjs/schedule` отложен до серий ≈2.9, «не потерять»). Следующее — `2.4·1`.**
...
> + **частичное выполнение** (≥minTarget = победа). Деплоится как `2.4.0` (·8).
...
**БД (2 шага):**
- [ ] **2.4·1 (БД: habits)** — схема `habits` (title/description?/icon?/domainKey?/`attributes[]`/
  goalId?/priority/kind `binary|quantitative|timed`/`recurrence` RRULE-строка/isActive/`ladder`
  {minTarget,currentTarget,goalTarget?,step?,policy `manual|adaptive`} — формат `ladder` (jsonb vs
  плоские колонки) зафиксировать при шаге) + интерфейс ADR-0033 + миграция. **Получим:** таблица
  привычек; пользователю не видно.
- [ ] **2.4·2 (БД: tasks)** — схема `tasks` (templateId? FK→habits/goalId?/title/`occurred_on`/kind/
  targetValue?/doneValue?/status `pending|done|partial|skipped`/skipReason?/postponedFromTaskId?/
  priority/category?/deadline?/completedAt?) + интерфейс + **уник `(templateId, occurred_on)`** +
  миграция. **Получим:** таблица задач дня.

**Привычки — бэк + API (3 шага):**
- [ ] **2.4·3 (Habits порт+репо)** — `HabitsModule` каркас; порт `ACCENT_HABIT_REPOSITORY` +
  Drizzle-репо (listByAccount/findOwned/create/update/deactivate, scope по account). **Получим:**
  доступ к данным привычек. Verified: tsc, boot.
- [ ] **2.4·4 (Habits domain)** — domain-service: CRUD + владение + инварианты лесенки
  (`minTarget≤currentTarget≤goalTarget`, step>0 при adaptive) + валидация RRULE-строки. **Получим:**
  бизнес-правила привычек. Verified: tsc, boot.
- [ ] **2.4·5 (Habits API)** — контроллер под Guard (`GET`/`POST /accent/habits`, `GET`/`PATCH
  /accent/habits/:id`, `POST /accent/habits/:id/deactivate`) + тонкие use-cases + closed-shape DTO
  (RRULE/ladder-валидация) + view. **Получим:** живое управление привычками. Verified live (CRUD, 401).

**Расписание + задачи — бэк (5 шагов):**
- [ ] **2.4·6 (RRULE-util)** — зависимость `rrule`; чистая util `isHabitDueOn(recurrence, date, tz)`
  (+ парс/валидация RRULE). Изолированно. **Получим:** «активна ли привычка в дату X». Verified: tsc
  + точечная проверка на нескольких правилах (daily / по будням / каждые N дней).
- [ ] **2.4·7 (Task порт+репо)** — порт `ACCENT_TASK_REPOSITORY` + Drizzle-репо (listByAccountOn(date)/
  findOwned/create/createMany/updateStatusFields, scope по account). **Получим:** доступ к данным
  задач. Verified: tsc, boot.
- [ ] **2.4·8 (ленивая материализация)** — domain `ensureTasksForDay(accountId, date, tz)`: для
  активных привычек, due по RRULE на дату и без инстанса на день — создать Task-снимок
  (`targetValue=currentTarget`, status=pending). Идемпотентно (уник `(templateId, occurred_on)`).
  **Получим:** задачи дня сами появляются из привычек при чтении. Verified: tsc, boot.
- [ ] **2.4·9 (выполнение: complete/uncomplete)** — domain: `complete(taskId, doneValue?)` →
  done/partial (**`partial≥minTarget` = победа**), идемпотентно, ставит completedAt; `uncomplete`
  → pending (revoke очков — TODO 2.9). Инварианты (done/partial требует doneValue+completedAt).
  **Получим:** задачи можно отмечать, в т.ч. частично. Verified: tsc, boot.
- [ ] **2.4·10 (перенос + разовые)** — domain: `postpone(taskId)` → создаёт задачу на завтра,
  текущая `skipped/postponed` (`postponedFromTaskId`); создание разовой (one-off, templateId=null,
  category?/deadline?). **Получим:** перенос задачи и разовые дела. Verified: tsc, boot.

**Лесенка (1 шаг):**
- [ ] **2.4·11 (LadderEngine)** — domain-service: на `complete` подстраивает `currentTarget`
  (adaptive: N лёгких подряд → `+= step` ≤ goalTarget; срыв/тяжело → откат к minTarget; `manual` —
  не трогает); врезка в `complete`. Эмитит `ladder.raised/lowered` (очки — TODO 2.9). **Получим:**
  планка сама ползёт от 1 к 100 без обнуления — ядро адаптивности. Verified: tsc, boot.
- [ ] **2.4·12 (API задачи)** — `GET /accent/tasks?date=YYYY-MM-DD` (+ `overdue`/`due-today`),
  `POST /accent/tasks` (разовая), `POST /accent/tasks/:id/complete|uncomplete|postpone` + use-cases
  + DTO + view. **Получим:** дневной чеклист по API end-to-end. **Verified live (throwaway):**
  привычка → задачи дня → complete/partial → лесенка двигается → postpone → разовая. 401.

**Фронт (4 шага):**
- [ ] **2.4·13 (фронт: типы+api)** — `accent.types` (Habit/Task/Ladder/views) + методы `accent-api`
  (habits CRUD/deactivate, tasks list/complete/uncomplete/postpone/one-off). **Получим:** клиентский
  слой данных. Verified: tsc.
- [ ] **2.4·14 (фронт: «Шаблоны»)** — вкладка управления привычками: список + CRUD-модалка
  (название/категория/атрибуты/kind/лесенка min·current·goal/RRULE-пресеты daily·будни·каждые-N) +
  деактивация. **Получим:** пользователь заводит/правит привычки. Verified: tsc + prod-сборка.
- [ ] **2.4·15 (фронт: «Сегодня»)** — вкладка дня: чеклист задач + % дня; TaskCard (чекбокс binary /
  частичный ввод quantitative-timed / перенос); empty-state. **Получим:** пользователь ведёт день.
  Verified: tsc + prod-сборка.
- [ ] **2.4·16 (фронт: фидбэк лесенки + полировка)** — тост «планка выросла 🎉 / сегодня мягче»,
  отображение targetValue/doneValue, состояния recovery (минимум), мелкая полировка. **Получим:**
  видимая адаптивность. Verified: tsc + prod-сборка strictTemplates.

**Деплой (1 шаг):**
- [ ] **2.4·17 (деплой 2.4.0)** — `VERSION`→`2.4.0`, `release-2.4.0.md` + `RELEASE_NOTES` (broadcast),
...
- **2.4 привычки + задачи + лесенка:** ✅ детализировано выше (2.4·1–·17 — мелкая нарезка), решения зафиксированы.
