...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  Следующее — подфаза `2.2` (микро-победы, первый user-facing): нарезана на ·1–·7
  (БД→бэк CRUD→API CRUD→complete→стартовый набор→фронт→деплой 2.2.0 бандлем с 2.1).**
...
- [x] **2.0.0·2 (БД)** ✅ — `accent_settings` (PK=FK `account_id`; `paused_from?`; timestamps; FK cascade) + интерфейс ADR-0033 + Drizzle-схема + миграция `0003`. Накатана на dev (psql ✅). `overall_streak_threshold` отложен на 2.8 (вар. A). На прод миграция — при деплое 2.0.0 (после ·6).
...
- [x] **2.0.0·4 (API)** ✅ — `GET /accent/settings` + `POST /accent/pause|resume` (под AuthGuard) + use-cases (get/pause/resume) + view `AccentSettingsView`. `PATCH /accent/settings` отложен (его поле `overallStreakThreshold` — в 2.8; DTO не нужен — нет тела). Verified live (dev, throwaway+token): GET→`{accentPausedFrom:null}` (ленивое создание) → pause 204 → GET с датой → resume 204 → GET null; строка в БД. api-contracts §1 — синхронизировать (PATCH/threshold пометить как 2.8).
...
> Бэкенд-онли (фронт-селекторы появятся с потребителями 2.3/2.4). Read-only для клиента,
...
> геймификация (2.8) подпишется на событие `micro_win.completed`; пока complete только логирует.
- [ ] **2.2·1 (БД)** — схемы `micro_wins` (`title`, `category` enum
  `physical|mental|emotional|social|sensory|household`, `duration_seconds` ≤300, `energy_cost`
  1..3, `effect?`, `disabled_for_states?` jsonb UserState[], `is_active`, `account_id` FK,
  timestamps) + `micro_win_logs` (`account_id` FK, `micro_win_id` FK, `occurred_on` date,
  `created_at`). **Дневной лимит = partial-unique `(micro_win_id, occurred_on)`** (1 лог/день).
  Интерфейсы ADR-0033 (Full 1:1 с колонками) + миграция. Накатить на dev, проверить таблицы.
- [ ] **2.2·2 (бэк CRUD)** — `MicroWinsModule`: порт `ACCENT_MICRO_WIN_REPOSITORY` + Drizzle-репо
  (CRUD + scope по `account_id`) + domain-service (`list`/`create`/`update`/`delete`, владение
  аккаунтом, инварианты duration/energyCost). Импорт `AccessControlModule` (Guard+аккаунт).
  Зонтик `AccentModule` импортит. Verified live: deps init, health 200, tsc.
- [ ] **2.2·3 (API CRUD)** — контроллер под AuthGuard: `GET /accent/micro-wins`,
  `POST /accent/micro-wins`, `PATCH /accent/micro-wins/:id`, `DELETE /accent/micro-wins/:id`
  + тонкие use-cases + closed-shape DTO (без `any`, enum-валидация category/energyCost) + view
  (`MicroWinView` + флаг `completedToday`). Verified live: роуты, 401 без токена, CRUD-цикл.
- [ ] **2.2·4 (complete = лог)** — `POST /accent/micro-wins/:id/complete` Body `{ occurredOn? }`
  → domain-service `complete` (идемпотентно по `(microWinId, occurredOn)`, ON CONFLICT — повтор
  в тот же день no-op). `occurredOn` по умолчанию = **сегодня в TZ аккаунта** (use-case зовёт
  вниз `AccentSettingsDomainService` за TZ — кросс-домен вниз, ADR-0050). Эмитит доменное событие
  `micro_win.completed` (слушатель — в 2.8; сейчас просто публикуем). `completedToday` в list-view.
- [ ] **2.2·5 (стартовый набор)** — при первом обращении аккаунта к разделу — идемпотентно
  завести персональный стартовый набор MicroWin (напр. «выпить воды», «1 отжимание», «выйти на
  балкон», «5 вдохов»…) по `account_id` (ON CONFLICT/флаг в настройках, без дублей при повторе).
  В survival/recovery система потом рекомендует именно их (Recommender — 2.7). Verified live.
- [ ] **2.2·6 (фронт)** — lazy-feature `/accent/micro-wins`: список-чипы «сделать сейчас»
  (`MicroWinChip`), complete одним тапом с дневным фидбэком (✓ сегодня / дизейбл), создание/
  редактирование (MatDialog), удаление, EmptyState+CTA. Сервис `accent-api`, Signals, OnPush,
  чистый SCSS. Вкладка в шелле раздела. Verified live: CRUD+complete с UI, лимит виден.
- [ ] **2.2·7 (деплой 2.2.0 = бандл 2.1+2.2)** — `VERSION`→`2.2.0`, релиз-нота
  `seed-content/notifications/release-2.2.0.md` + запись в `RELEASE_NOTES` (broadcast),
  `make prod-build/up/migrate`, smoke на проде. Первый видимый прод-выкат фазы 2 после каркаса.

### 2.3–2.11 — скелет (детальные шаги — при старте подфазы)
