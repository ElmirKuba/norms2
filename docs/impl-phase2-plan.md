...
- [ ] **2.7·34** Бэк зачёта — **одна операция вместо двух**: use-case «сделать минимум»
  (`POST /accent/tasks/:id/complete-minimum`) в одной транзакции пишет лог микро-победы **и**
  закрывает задачу как `partial` с `doneValue = ladder.minTarget` (серия жива, лесенка не двигается
  вверх). Идемпотентность и CAS — как у обычного `complete` (F6–F8). API-смоук.
