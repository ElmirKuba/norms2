...
export interface MicroWinFull {
  ...
  /**
   * Сфера жизни (мягкий ключ, опц.; ось «какую сферу питает») — общая со целями/привычками
   * (ADR-0056, M#B3-1: две оси). null = не указана. Без FK, как `domainKey` у целей/привычек.
   */
  domainKey: string | null;
  /** Длительность действия в секундах (само время действия таймера; ≤ ~300). */
  durationSeconds: number;
  /**
   * Время на подготовку в секундах (M#B3-4, опц.): отсчёт ПЕРЕД действием (нажать ▶, отложить
   * телефон, встать в планку); по нулю — звук «старт». null = без подготовки (сразу действие).
   */
