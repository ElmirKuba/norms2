...
/**
 * Убирает у ноты первый заголовок первого уровня.
 *
 * Каждая релиз-нота начинается с `# Заголовок`, и он же лежит в поле `title` — то есть уже
 * показан в шапке модалки и страницы витрины. Без этой обрезки заголовок печатается дважды
 * подряд, что выглядит как ошибка вёрстки. Режем только **самый первый** `# ` и только если
 * он в начале текста: `#` внутри ноты — часть содержания и остаётся.
 *
 * @param source Исходный Markdown.
 * @returns Текст без ведущего H1.
 */
