...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  `2.4·21` ✅ **деплой — в проде 2.4.19** (smoke 200/200; lock-фикс @emnapi по пути), `2.4·22` ✅ ретро
  0.0→2.4 ([retro-doc](./retro-phase2-0.0-2.4.md); долг `2.4·23` version/конкурентность — отложен).
  **Вся фаза 2.4 закрыта и в проде.** Следующее — **`2.5·1` (DESIGN «Цели», вариант B)** — детальная
  нарезка 2.5·1–·23 ниже; ключ по конкурентности: агрегаты вычисляем, version не нужен.**
...
**Долг из ретро (отложено, не сейчас — альфа):**
- [ ] **2.4·23 (довести Accent до ADR-0035 — конкурентность)** — подтверждено аудитом
  ([audit-phase2-security.md](./audit-phase2-security.md), топ-3): (1) version-CAS на запись лесенки
  (`accent-ladder-engine`/`setLadder`); (2) идемпотентность `complete` — двигать лесенку только при
  реальной смене статуса (условный UPDATE + rowCount), не по снимку `wasOpen`; (3) `postpone` в транзакцию.
  Долг (реш. Elmir 2026-06-22: не сейчас).
- [ ] **2.4·24 (security-хардненинг, из аудита)** — `JWT_ACCESS_SECRET ≥ 32` (⚠️ сперва ротировать
  прод-секрет, иначе fail-fast) + удалить мёртвый `JWT_REFRESH_SECRET`; `helmet` + запрет `CORS_ORIGIN="*"`.
  Дёшево; #4 (слабый секрет) — самый серьёзный по сути. **P1 «восстановление» — отложено post-alpha.**
...
**Дизайн (1 шаг):**
- [ ] **2.5·1 (DESIGN)** — 3 шляпы + ADR. Решить: **семантика значения цели** (`direction`:
  `accumulate` накопит. «50 книг» / `reach` дойти до «вес 70» / `reduce` снизить «курить 0») —
  от неё формулы `currentValue/%/forecast`; принцип **вычисляемых агрегатов** (см. 🔧); тон forecast
  («при темпе будешь к ДАТЕ», не «отстаёшь»); подтвердить полную нарезку ниже. Deliverable: ADR + финал нарезки.

**Бэк — ядро цели (5 шагов):**
- [ ] **2.5·2 (бэк)** — схема `goals` (+`parent_goal_id`, +`direction`) + миграция; `GoalFull`-интерфейс. 🔧 без `version`.
- [ ] **2.5·3 (бэк)** — порт+Drizzle-репо целей: CRUD + `listByAccount(status?, domain?)`.
- [ ] **2.5·4 (бэк)** — domain-service: create/update/get/list + инварианты (`target>0`, `direction` валиден,
  глубина ≤ `ACCENT_GOAL_MAX_DEPTH` из конфига → `GOAL_MAX_DEPTH_REACHED` 422).
