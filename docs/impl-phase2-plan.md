...
- **▶ Следующее (по факту на 2026-07-31):** подфаза **`2.7` «Препятствия»** закрыта целиком (блоки 0–J,
  ...
  строки явный `replace` на понижение значения. Дальше по порядку хвоста фазы 2 —
  **подфаза 2.11 «дашборд»** (выйдет версией `2.8.0`), затем 2.8 «чек-ин» (версией `2.11.0`).
...
> ⚠️ **Порядок выполнения ≠ порядок номеров (реш. Elmir 2026-07-31).** Дашборд делается сразу
> после 2.7, а чек-ин — после него: **2.7 → 2.11 → 2.8 → 2.9 → 2.10 → 2.12**. Номера подфаз
...
- [x] **2.7·29** ✅ Порядок хвоста: **2.7 → 2.11 (дашборд) → 2.8 (чек-ин) → 2.9 (очки) → 2.10 (неделя) → 2.12**.
...
> **Нота на патч, и только про новое.** Колокольчик — журнал изменений, а не справочник.
>
...
7. **Движение планки видно со следующего дня, а не в день отметки.** `tasks.target_value` — снимок
...
8. ~~**Причина пропуска `cancelled` была мертва**~~ — объявлена в типе и не писалась никогда.
