...
- [ ] **·5 (бэк)** — Telegram-адаптер: `sendPhoto` (превью + подпись со ссылкой, лимит **1024**) → `sendMessage` с полным текстом, разрезанным **по границам разделов `##`** при превышении **4096** (наши ноты бывают до ~7100 символов). Обработка `429 retry_after`, «бот не админ», глушение ошибок доставки.
- [ ] **E8 [Elmir]** — проверить первый пост в тестовом канале.
- [ ] **·6 (бэк)** — экранирование под разметку Telegram, переносы, обрезка.
