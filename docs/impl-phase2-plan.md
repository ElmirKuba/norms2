...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.5 «Цели» В РАБОТЕ:** `2.5·1` (DESIGN) ✅ — [ADR-0052](./decisions/0052-accent-goal-direction-and-computed-progress.md)
  ...
  boot/DI зелёный, таблица создана). **СЛЕДУЮЩЕЕ — `2.5·8` (entries domain: `addEntry` с `GOAL_PAUSED` 409 +
  авто-`completed` conditional-update; событие `goal.completed` отложено до 2.9) → `2.5·9` (вычисляемые
  currentValue/%/daysLeft/pace/forecast в GoalView + endpoints `POST`/`GET` entries cursor; live-forecast для
  3 direction).** Нарезка ·8–·23 ниже.
