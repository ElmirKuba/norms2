...
- [ ] **E1** — бот заведён 04.08.2026: **`@norms2_bot`**. Осталось положить `TELEGRAM_BOT_TOKEN` в `.env` (dev и прод).
  - ⚠️ **Первый токен отозван в тот же день:** он был отправлен в переписку с ассистентом и тем самым осел в истории чата. Перевыпуск — `/revoke` у `@BotFather`. Это ровно тот риск, из-за которого в шапке блока стоит «токен не называется в переписке»: утёкший токен даёт полный контроль над ботом, а бот умеет выдавать приглашения.
...
- [ ] **E3** — добавить бота в канал **администратором** с правом публикации.
- [ ] **E4** — прописать `TELEGRAM_CHANNEL_ID=@имя_канала` (на dev — **тестовый** канал) и `TELEGRAM_OWNER_CHAT_ID` (свой чат с ботом; узнаётся после первого `/start`).
