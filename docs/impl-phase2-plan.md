...
- **Нарезка (сперва дока, потом фронт; бэкенд не трогаем) — resumable подпункты:**
  - **H1·1 (docs):**
    - [ ] H1.1a `ui-ux`/`api-contracts`: доступность таймера для `timed`-привычек (кнопка «▶ засечь» на карточке задачи), источник длительности = `ladder.currentTarget` с ad-hoc-override.
    - [ ] H1.1b зафиксировать семантику завершения: доотсчёт → «зачесть» → существующий `complete(performed=секунды≥target)`; cancel — ничего.
    - [ ] H1.1c мини-ADR «единый таймер-компонент для трекеров» (микро-победы + привычки).
  - **H1·2 (frontend)** — эталон: `angular/src/app/features/accent/micro-wins/micro-win-timer-modal.component.ts`:
    - [ ] H1.2a вынести таймер-модалку в общий `accent`-компонент (generic по данным), микро-победы продолжают работать (регресс-проверка).
    - [ ] H1.2b «▶ засечь» на карточке `timed`-задачи привычки → открыть таймер с `currentTarget` (+ поле override).
    - [ ] H1.2c провод завершения → существующий эндпоинт `complete(performed)`; обновить вью без перезагрузки.
  - **H1·3 (verify):**
    - [ ] H1.3a создать `timed`-привычку, прогнать таймер до конца → задача `done`, лесенка сместилась.
    - [ ] H1.3b отмена таймера ничего не пишет; микро-победный таймер не сломан; `ng build` (prod) зелёный.
...
- **Нарезка — resumable подпункты (⚠️ трогает ядро `LadderEngine` — делать аккуратно, с ADR первым):**
  - **H2·1 (docs+ADR):**
    - [ ] H2.1a ADR: полярность лесенки `direction: raise|lower` + `clock`-kind (минуты от вечернего якоря, midnight-safe); инвариант при `lower` переворачивается (`goalTarget ≤ currentTarget ≤ minTarget`).
    - [ ] H2.1b правки `domain-model §5` (kind+ladder), `gamification §7` (ветка движка), `api-contracts §5`.
...
- **Нарезка (B/S/A/verify) — resumable подпункты:**
  ...
  - **2.6·C3 (домен+API)** — эталон: слайс `modules/accent/goals/` (свежий полный):
    ...
    - [ ] C3.2 `interfaces/anti-habit-full.interface.ts` + `anti-habit-view.interface.ts`.
  ...
  - **2.6·C4 (фронт)** — эталон: `angular/src/app/features/accent/habits/` и таймер микро-побед:
    ...
    - [ ] C4.3 список-компонент (карточки + «+добавить», пустое состояние).
    - [ ] C4.4 форма create/edit (`title`/`description`/`targetDays`) в `MatDialog` на общем каркасе `.dlg`.
    - [ ] C4.5 деталь: **живой счёт-вверх** дн:ч:мин:сек (`setInterval` + очистка в `ngOnDestroy`), circular-ring к `targetDays`.
    - [ ] C4.6 «Сорвался»: подтверждение + поле `triggerTag`/`note` → relapse → обновить ВСЕ производные вью без перезагрузки.
    - [ ] C4.7 история попыток (cursor-пагинация).
    - [ ] C4.8 копирайт non-punitive («Срыв»→«Новая попытка», рекорд на виду).
  - **2.6·C5 (verify)** — live-смоук на прод-деве:
    - [ ] C5.1 создать держусь (`targetDays=7`); таймер идёт вверх (дн:ч:мин:сек, ring растёт).
    - [ ] C5.2 срыв → сброс, `attemptNumber++`, запись попытки с `attemptDurationMs`, **рекорд сохранён/обновлён**.
    - [ ] C5.3 повторный «срыв» в тот же момент/без активной попытки → `ALREADY_RELAPSED`.
    - [ ] C5.4 история пагинируется; `tsc` + `ng build` (prod) зелёные.
