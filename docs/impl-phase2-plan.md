...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  `2.2·2` ✅ (бэк CRUD: порт+репо+domain-service) и `2.2·3` ✅ (API CRUD под AuthGuard, CRUD-цикл
  verified live). Следующее — `2.2·4` (complete=лог: идемпотентно по дню, `occurredOn` по TZ
  аккаунта, событие `micro_win.completed`, `completedToday` в list-view).**
