...
**Блок G — ренумерация подфаз 2.9/2.10/2.11 (docs-работа, НЕ код 2.7; реш. Elmir 2026-07-29)**
> **Зачем:** дашборд превращает пять вкладок в продукт с входной точкой дня, а очки и недельный
> планер — надстройки, которые ждут. Поэтому дашборд переезжает сразу после 2.8. **Почему блок
> лежит здесь, а не выполняется сейчас:** решение принято, но лимиты недели исчерпаны; чтобы не
> держать это в памяти и не потерять — оно живёт шагом плана и будет видно, когда дойдём.
> **Почему после кода, а не до:** ренумерация правит ссылки по всей доке, а блоки A–F этот же
> документ активно меняют — сначала стабилизируем содержимое, потом номера.
> **📏 Оценка масштаба (разведка 2026-07-30, сам блок НЕ выполнялся):** ~85 упоминаний
> «2.9/2.10/2.11» в ~14 файлах. Разбивка: **план — 55** (таблица подфаз, таблица таблиц БД,
> скелет 2.6–2.12, зависимости, россыпь по тексту); **`docs/sections/accent`** — 10
> (domain-model 5, api-contracts 4, ui-ux 1); **ADR** — 10 (0049/0056/0059/0062); **код** — 26,
> все в **комментариях** (порт и адаптер событий «Держусь» — 9, micro-wins/habits/ladder-engine/
> milestone — 7, модули — 3, дашборд-компонент — 2 и др.). **Retro-файлы (7 упоминаний) НЕ трогаем**
> — это исторические записи о том, как было. В коде «2.9» почти везде = геймификация → станет 2.10;
> «2.11» в дашборд-компоненте → станет 2.9. Объём: один средний заход, работа механическая, но
> каждое попадание надо смотреть глазами (см. ловушку про номера секций ниже).
>
> **⚠️ Альтернатива, которую стоит рассмотреть ПЕРЕД правкой:** менять не номера, а только
> **порядок выполнения**. Номера подфаз — внутренние ярлыки, а не версии релизов; ничто не мешает
> делать 2.11 (дашборд) сразу после 2.8, оставив ему номер 2.11. Тогда вместо ~70 правок в 14
> файлах достаточно одной строки в плане («порядок: 2.8 → 2.11 → 2.9 → 2.10 → 2.12»), и ноль
> шансов зацепить номер секции доки или исторический retro. Минус — номера перестают идти по
> порядку выполнения, что чуть путает при чтении плана. **Решает Elmir.**

- [ ] **2.7·29** Ренумерация: **дашборд-агрегатор → новый `2.9`** (был 2.11, встаёт сразу после 2.8),
  **геймификация → новый `2.10`** (была 2.9), **недельный слой → новый `2.11`** (был 2.10); `2.12`
  (поддержка) не двигается. Правим: таблицу подфаз, таблицу «какие таблицы БД в какой подфазе»,
  скелет «2.6–2.12», зависимости в строках подфаз.
- [ ] **2.7·30** Консистентность ссылок: `grep -rn "2\.9\|2\.10\|2\.11" docs/ nest/ angular/` и выверка
  каждого попадания — план, `docs/sections/accent/*.md`, ADR (0027/0048/0051/0052/0053/0056/0059/
  0060/0062 ссылаются на номера подфаз), TODO-маркеры в коде (как было с `micro-win-full.interface.ts`).
  ⚠️ **Ловушка:** в `api-contracts`/`domain-model`/`ui-ux`/`README` «2.N» часто означает **номер
  секции доки**, а не подфазу — их трогать НЕЛЬЗЯ (эта грабля уже ловила нас при прошлой
  ренумерации). Отличать по контексту, а не заменой по маске.
...
> **G и I — docs-шаги** (ренумерация и ADR), **H — код**; порядок именно такой, чтобы ренумерация
> правила уже стабилизировавшийся текст.
