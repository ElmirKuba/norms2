...
/**
 * Персональный стартовый набор микро-побед — заводится **по кнопке** «Получить пак»
 * (`POST /accent/micro-wins/starter-pack`), идемпотентно по дедупу названий (только
 * докидывает недостающие, своё не трогает). Авто-сева при первом доступе нет.
