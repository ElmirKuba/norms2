...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Код: 2.0.0·1 ✅ (модуль), 2.0.0·2 ✅ (таблица accent_settings, миграция 0003 на dev).
  Следующее — `2.0.0·3` (порт+репо+`AccentSettingsDomainService`: get/pause/resume).**
  На прод НЕ деплоим до 2.0.0·6 (каркас невидим — реш. Elmir); прод-миграция 0003 — при том деплое.
...
- [ ] **2.0.0·3 (бэк)** — `AccentSettingsModule` (`modules/accent/settings/`, импортит `AccessControlModule`, экспортит domain-service; зонтик `AccentModule` его импортит): порт + Drizzle-репо + `AccentSettingsDomainService` (getOrCreate/pause/resume). Интерфейс → `settings/interfaces/`.
