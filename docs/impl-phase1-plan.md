...
- [ ] **I2** — InviteTree (рекурсивный CTE) + bans + бан-чек логина, снизу вверх. NB: таблица `bans` (FK restrict, индекс `(target_id,active)`, partial-unique `(banner_id,target_id) WHERE active`) **уже в миграции 0000** — схема/drizzle-kit не нужны; из bans-интерфейсов есть только `ban-full`.
  ...
  - [ ] **I2.4** — login учитывает бан (**TODO A3.3**): `AccountBannedError` (детали = активные баны: банивший+причина, ADR-0012) + `LoginAccountUseCase` оркестрирует `authenticate`↓ + `bans.listActiveByTarget`↓ → при наличии бросает с деталями; ответ заблокированного логина в `api-contracts.md`.
  - [ ] **I2.5** — Guard учитывает бан (**TODO A3.5**) + **ADR**: где живёт кросс-доменный ban-чек на каждый защищённый запрос (выделенный access-decision use-case/сервис vs Guard как лёгкий оркестратор account+bans) — развилка → ADR-0038; затем реализация и live-проверка (бан в окне access-токена блокирует).
