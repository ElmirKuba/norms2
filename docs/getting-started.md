...
Раздел `/admin` открыт только аккаунтам с ролью `admin`. Роль **не выдаётся автоматически никому**
и не задаётся в конфиге: логин администратора не должен жить в репозитории открытого проекта.

Порядок такой:

1. **Зарегистрируйтесь в «Нормисах» обычным способом.** Скрипт выдаёт роль существующему
   человеку, а не заводит его. Роль `user` при регистрации выдаётся сама.
2. **Запустите скрипт** — он спросит окружение (если запущено несколько), уточнит, зарегистрирован
   ли аккаунт, и попросит логин:

```bash
./scripts/grant-admin.sh
```

На Windows — `scripts\grant-admin.cmd` (то же самое, тот же SQL).

Можно передать логин сразу: `./scripts/grant-admin.sh мой_логин`.

**Что он делает:** заходит в работающий контейнер PostgreSQL (`norms2_postgres_dev` или
`norms2_postgres_prod`) и выполняет SQL — находит аккаунт по логину без учёта регистра, находит
роль `admin` в справочнике, связывает их и пишет строку в журнал действий. Повторный запуск
ничего не портит: если роль уже есть, он так и скажет.

**Права действуют сразу**, перезапуск не нужен — роль читается из базы на каждом запросе. По той
же причине снятие роли действует мгновенно.

**Если скрипт ругается, что роли `admin` нет в справочнике** — значит бэкенд ещё ни разу не
стартовал на этой базе. Поднимите его (`make dev-up`), он засеет справочник ролей, и повторите.
