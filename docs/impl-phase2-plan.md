...
- [ ] **·3 (бэк, постоянство)** — `AccentPersistenceDomainService`: итог «дней с действием» и окно «X из последних N», общий по аккаунту и по привычке. **День засчитан = любое зафиксированное действие** (отметка задачи, микро-победа, запись в цель) — порога «≥70% задач дня» нет, он возвращает «всё или ничего». Считается **проекцией**: репозиториям нужны запросы «дни с активностью в диапазоне», по одному на источник, объединение — в домене.
