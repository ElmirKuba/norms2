...
- **2.3 стартовый набор (отличимость + adoption) ← СЛЕДУЮЩЕЕ, детализировать при старте:**
  БД — колонка `micro_wins.is_starter` (миграция). Бэк — сид флагует `is_starter=true`;
  «принятие» (adoption) снимает флаг = победа становится «своей»; эндпоинт «получить пак»
  (re-pull/сид по запросу). Фронт — badge «стартовое» на непринятых, кнопка «Получить
  стартовый пак», «Сделал» на стартовой → adoption (помечает своей), остальные остаются
  стартовыми. Суть: онбординг практикой, пустого экрана нет, но выбор явный («UI-UX отдача»).
  **Открытые вопросы (решить при старте):** триггер adoption — только «Сделал» или ещё
  «Изм.»/явная «оставить»? когда показывать кнопку — всегда / только когда стартовых нет?
  авто-сид при регистрации (как сейчас) vs только по кнопке? нужна ли «очистить примеры».
