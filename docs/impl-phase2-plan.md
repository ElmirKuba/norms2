...
- [ ] **·7 (БД + бэк)** — колонка `tasks.ladder_before` (jsonb, nullable) + миграция; `complete` кладёт снимок лесенки, **только** если реально её двигал (adaptive + переход из открытого).
