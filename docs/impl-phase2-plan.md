...
- **▶ Следующее (план от 2026-07-29):** сначала патч **`2.6.4` — свежесть данных между устройствами**
  ([ADR-0061](./decisions/0061-cross-device-data-freshness.md), нарезка F1–F12 в блоке «2.6.4» ниже;
  боль: отметил с телефона — на десктопе стейл до F5, а повтор клика **тихо затирает** значение),
  затем подфаза **`2.7` «Препятствия»** — детальная нарезка на 29 под-шагов готова (блок «🧩 2.7»).
...
#### 2.6.4 — свежесть данных между устройствами (догфудинг Elmir, 2026-07-29) — ⬜ не начато
...
> **Почему это патч ДО 2.7:** механизм сквозной; препятствия должны родиться уже с ним, а не
> получать его задним числом.
>
...
> Идёт **после патча 2.6.4** (свежесть данных), чтобы экраны препятствий сразу были кросс-девайсными.
...
**Блок A — БД**
- [ ] **2.7·3** `database/schemas/obstacles.schema.ts` — `account_id`, `name`, `type?`, `trigger?`,
...
- [ ] **2.7·4** `schemas/counterplays.schema.ts` — FK `obstacle_id` `ON DELETE CASCADE`, `text`,
  `linked_micro_win_id?` (`ON DELETE SET NULL`), `position`, времена; индекс `(obstacle_id, position)`.
