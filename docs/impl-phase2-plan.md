...
- [ ] **2.7·37** ADR-0063: **ни одна функция раздела не деградирует при отсутствии ИИ.** Зафиксировать:
  (1) `Recommender` 2.8 — **rule-based** (входы: состояние из чек-ина, `energyCost`,
  `disabledForStates`, история выполнений, время суток, просрочки, ближайшие вехи; взвешенный
  скоринг, детерминированно); (2) **объяснимость как преимущество** — правило честно говорит «почему
  предложил», LLM этого не гарантирует, а требование объяснимости уже записано в `api-contracts §14`;
  (3) обучение **без ML** — частотная статистика «предложено → сделано» + epsilon-greedy (десятки
  строк, ноль библиотек, ноль токенов); (4) LLM оправдан только на текстовой работе (декомпозиция
  цели, перепаковка срыва) и живёт **за фича-флагом**: нет ключа — кнопки просто нет; (5) **приватность
  как главный аргумент, а не деньги** — отправка формулировок целей и заметок о срывах во внешний API
  = передача пользовательского контента третьей стороне, против self-hosted-принципа и логики
  минимизации данных ([ADR-0001](./decisions/0001-data-minimization-no-pii.md)); (6) локальный
  инференс: 8 ГБ VRAM на дев-машине хватает для квантованной 7–8B, но на прод-VDS GPU нет →
  для пользователей не масштабируется, только дев-режим. Плюс индекс в `decisions/README.md`.

**Блок J — закрытие**
- [ ] **2.7·38** Финал: браузер-verify Playwright (список/форма/деталь/«Столкнулся»/drag/стартер-пак/
  связь с рецидивом/минимум-кнопка, ноль ошибок консоли) + **паспорт фичи `docs/feature-obstacles.md`
  переписывается в настоящем времени по факту** (заведён в ·2b; заодно фиксируем, сколько пришлось
  переписать — метрика качества аналитики) + раздел в накопительной ноте → `VERSION`→`2.7.0` → бандл-деплой на Johanna + smoke + broadcast.
