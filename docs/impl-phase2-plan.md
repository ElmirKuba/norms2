...
- [ ] **·10 (фронт, типы + сервис)** — типы истории, метод `AccentApiService`.
- [ ] **·11 (фронт, экран)** — компонент детали привычки, роут `habits/:id`, **честный пустой стейт**: «показать пока нечего, возвращайся позже» — без обещаний, когда именно (реш. Elmir).
