...
- [ ] **2.5·23 (ТРИАЖ «цели не совсем работают») — СЛЕДУЮЩИЙ.** Elmir потыкал ночью (без глубокого
  вникания) — ощущение, что цели работают не до конца. **Быстрый багхант-триаж ДО постройки 10/10-фич
  (не строим на трещине).** Пройти ключевые потоки: создание всех 3 родов (accumulate/reach/reduce),
  запись/правка/удаление записи, %/forecast/прогноз-даты, подцели+rollup, вехи, lifecycle (пауза/архив/
  восстановление/авто-complete), привычка→цель (вклад/откат), стартер-пак (seed/adopt/clear), дашборд.
  **DoD:** список «симптом→причина» зафиксирован; найденные блокеры починены; smoke зелёные; что не блокер —
  занесено в ·27. (Если триаж вскроет много — часть фиксов может стать своими под-пунктами ·23.x.)
