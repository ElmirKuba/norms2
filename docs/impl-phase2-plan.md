...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  и `2.2·2` ✅ (`MicroWinsModule`: порт+Drizzle-репо+domain-service, CRUD с владением+инвариантами).
  Следующее — `2.2·3` (API CRUD: контроллер под AuthGuard + use-cases + closed-shape DTO + view).**
