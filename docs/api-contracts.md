...
## Техническая админка (2.9.3·5) 📋 — контракт, реализации ещё нет

> **Статус на 10.08.2026: спроектировано, не написано.** Готовы фундамент и правила — роли
> (`roles`/`account_roles`, 2.9.3·0), сид и назначение админов (·2), `@Roles('admin')` +
> `RolesGuard` (·3), рантайм-настройки (·4). Ручек ниже в коде **нет**; раздел описывает, какими
> они будут, и фиксирует решения до того, как их начнут писать. Реализация приходит вместе с
> фронтом админки; ·6 и ·7 приносят свои куски (журнал и удаление публикаций).
>
...
- `DELETE /admin/accounts/:id/roles/:code` (admin) → 200 `AdminAccountView`.
  ...
  - ⚠️ **Снятие `admin` не переживёт перезапуск, если логин остался в `ADMIN_LOGINS`.** Сид ·2
    роли **только выдаёт** (это его правило: опечатка в `.env` не должна молча разжаловать
    администратора), поэтому на следующем старте роль вернётся. Снятие через админку — половина
    операции; вторая половина — убрать логин из `.env`. Ответ на `DELETE` поэтому несёт флаг
    `willBeRestoredBySeed: boolean`, чтобы админка сказала об этом вслух, а не оставила
    расходящееся состояние на потом.
...
- `PUT /admin/settings/:key` (admin) Body `{ value: string }` → 200 `SettingView`.
  ...
  - **Переключение действует сразу, без перезапуска** — ради этого настройки и переехали в базу
    (·4). ⬜ Именно это ·4 доказать не смог: вызывать было некому. **Первый живой прогон этой
    ручки и есть недостающая проверка** — снять с ·4 отметку «не доказано» можно только после неё.
...
- `DELETE /admin/releases/:key` (admin) → 204 (·7). Каскад уносит доставку и отметки прочтения
