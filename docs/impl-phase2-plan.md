...
- [ ] **·3 (бэк, домен: дни)** — сборка ленты из строк: `done`/`partial` (со значением против планки и временем), `skipped/postponed`, `pending`. **Склейка переноса**: сегодняшняя `skipped/postponed` + завтрашняя копия → **один объект** «перенесено на завтра» (бизнес-логика на своём слое, фронт сырьё не разгребает).
- [ ] **·4 (бэк, домен: планка)** — восстановление движений лесенки из последовательности `target_value` соседних дней → события «планка выросла 8 → 10» / «стало мягче». Факт, без механики (D).
- [ ] **·5 (бэк, домен: тишина)** — «последняя отметка N дней назад» и счёт условных пропусков **на лету** по текущему расписанию, без единой новой строки. Оговорка в коде: расписание могли менять, это **оценка**, не факт.
