...
- [ ] **F4 (фронт, ядро свежести)** — `core/freshness/data-freshness.service.ts`: слушатели `visibilitychange`/`focus`/`online`, троттл 10 с, «свежесть» 5 с, счётчик занятости (открытый `MatDialog`, in-flight мутация, идущий таймер, dirty-форма) с отложенным обновлением. **Без реестра секций** — один колбэк на экран.
