 ...
 *
 * ⚠️ **Обратная слепота (замечена 2026-08-01):** поиск идёт по **имени**, а не по паре
 * «интерфейс.поле». Стоит одному экрану показать `completedAt`, и детектор замолкает про
 * `completedAt` во **всех** проекциях — включая те, где потребителя по-прежнему нет. Так из
 * отчёта тихо исчезли `GoalView.completedAt` и `TaskView.completedAt`. Лечится либо разбором
 * AST, либо привычкой не доверять «пусто» без глаз.
 *
