...
- [ ] **·11 (рендерер нот)** — решение Elmir 2026-08-04: **пишем своё, без библиотек**, закрытым набором конструкций (не весь CommonMark). Добавляем: инлайн-код, вложенные списки, цитаты `>`, таблицы GFM, нумерованные списки. Осознанно НЕ поддерживаем: картинки, сырой HTML, блоки ```` ``` ````, заголовки глубже `###`, зачёркивание, чекбоксы, сноски — набор записывается правилом в шапке `md-render.util.ts`, чтобы автор ноты знал границу. XSS-щит прежний: экранируем всё, потом накладываем разметку. Отвергнуто: `marked`(~35 КБ)/`markdown-it`(~100 КБ) **плюс обязательный `DOMPurify`**(~20 КБ) — готовые парсеры отдают HTML как есть.
  - **Почему набор именно такой:** в живых нотах уже используются вложенные списки (`release-2.8.0.md`) и инлайн-код — и то, и другое сейчас рендерится сломанным; из-за отсутствия таблиц ноту 2.9 переписывали списком (коммит `2c54925`).
