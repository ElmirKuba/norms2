...
- [ ] **F1 (docs)** — синхрон `api-contracts §5` (новый исход `409 TASK_ALREADY_COMPLETED`) + новый §15 «Ревизия/свежесть» (`GET /accent/rev`), ссылка на ADR-0061 в `architecture.md`/`frontend.md`.
- [ ] **F2 (БД)** — таблица `accent_revisions` (`account_id` PK, `rev` jsonb `{section: number}` или колонки по секциям, `updated_at`) + миграция `00XX` → просмотр SQL → `db:migrate` на dev.
- [ ] **F3 (бэк, порт+репо)** — `RevisionBumperPort` (`bump(accountId, section)`) + `read(accountId)`; Drizzle-реализация (upsert `rev = rev + 1` атомарно, без гонок), бинд в модуле.
- [ ] **F4 (бэк, интерсептор)** — `RevisionBumpInterceptor` на мутирующих `/accent/*` (успешный `POST/PATCH/PUT/DELETE` → bump секции из первого сегмента пути). Домены о ревизии не знают.
- [ ] **F5 (бэк, чтение)** — `GET /accent/rev` (use-case + контроллер): отдаёт все секции одним объектом; отсутствующая секция = 0.
- [ ] **F6 (БД, версия задачи)** — колонки `tasks.version` (`integer not null default 0` — как у `habits`/`anti_habits`) и `tasks.updated_at` (её сейчас нет вообще) + миграция `00XX` (аддитивная) → просмотр SQL → `db:migrate` на dev; `version` в `TaskFull`/`TaskView`.
- [ ] **F6b (БД, версия остальных правимых сущностей)** — той же миграцией `version integer not null default 0` для `micro_wins`, `milestones`, `goals`, `accent_settings` (конвенция — [ADR-0035, уточнение 2026-07-29](./decisions/0035-concurrency-control.md)). Колонку заводим сразу + bump в репозиториях; строгий CAS включаем только там, где гонка реальна (`tasks` — F7). **Не заводим** в append-only (`goal_entries`, `micro_win_logs`, `anti_habit_events`, `notifications`), справочниках и служебных таблицах. Для `goals` — уточнение к ADR-0052: агрегаты по-прежнему вычисляются на чтение, `version` только для PATCH полей цели.
- [ ] **F7 (бэк, CAS)** — все правки задачи идут по optimistic `version` ([ADR-0035](./decisions/0035-concurrency-control.md)): `UPDATE … WHERE id=… AND version=…`, `version+1` в **каждом** пути (complete / uncomplete / postpone / skip / patch) — единая точка в репозитории задач. Не нашли строку → `409 TASK_VERSION_CONFLICT` с актуальной задачей в теле. `updateIfOpen` (идемпотентность лесенки) остаётся как есть — CAS его дополняет, а не заменяет.
- [ ] **F8 (бэк, контракт)** — `version` в DTO `complete`/`uncomplete`/`postpone` (обязательное поле), ошибка `task-version-conflict.error.ts` (409, тело с актуальной задачей), синхрон `api-contracts §5`. API-смоук: стейл-`version` → 409 и `doneValue` **не** затёрт; накопительный сценарий таймера (свежий `version`) → 200; гонка 5× → один 200 + четыре 409, без 5xx.
- [ ] **F9 (фронт, ядро)** — `core/freshness/data-freshness.service.ts`: слушатели `visibilitychange`/`focus`/`online`, троттл 10 с, «свежесть» 5 с, реестр `register({section, reload})`, счётчик занятости (`MatDialog.openDialogs`, in-flight мутации, идущий таймер, dirty-форма) с отложенным рефетчем.
- [ ] **F10 (фронт, ревизия)** — сервис дёргает `GET /accent/rev`, сравнивает с последним снимком и вызывает `reload()` только изменившихся секций; тихо, без спиннера, точечным патчем сигналов.
- [ ] **F11 (фронт, вкладки)** — `BroadcastChannel('norms2')`: после своей мутации вещаем секцию; другие вкладки помечают стейл и обновляются (видимые — сразу, фоновые — при фокусе).
- [ ] **F12 (фронт, подключение экранов + 409)** — регистрация секций на «Сегодня»/привычках, целях, микро-победах, «Держусь»; проброс `version` во все вызовы задачи (одна точка — `AccentApiService` + `completeTask` в `habits.component`, у компонента `task` всегда свежий); обработка `409`: **без F5** — берём актуальную задачу из тела, патчим карточку, мягкое «уже отмечено на другом устройстве — обновили»; красная ошибка только когда записанное значение расходится с введённым.
- [ ] **F13 (verify)** — два браузерных контекста Playwright (эмуляция «телефон + десктоп»): отметка в одном → фокус во втором → карточка обновилась без F5; клик по стейл-карточке → 409, значение **не** затёрто, экран сам показал правду; накопительный таймер и «Начать сначала» по-прежнему работают; открытая модалка откладывает рефетч; `tsc`+`ng build` зелёные.
- [ ] **F14 (релиз)** — `VERSION`→`2.7.1`, дописать раздел в накопительную ноту линии 2.7 (`release-2.7.0.md` → переименовать файл/ключ на `release-2.7.1`), деплой + smoke.
