...
- [ ] **2.7·24** **«Столкнулся»** — главный поток: кнопка на карточке и детали → модалка «что
  делаем» со списком своих контрмер (выбрал → записан encounter с `counterplayId`; контрмера с
  `linkedMicroWinId` **сразу открывает таймер** микро-победы, ADR-0057; «просто отметить» → без
  контрмеры; опц. заметка) → счётчик и лента обновляются без перезагрузки. Ретро-исход: в ленте у
  записи без `outcome` — ненавязчивое «Помогло?» `да / не очень / ×` (можно пропустить навсегда).
