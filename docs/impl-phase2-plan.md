...
- **Ручная сортировка трекеров (drag-to-reorder)** — сквозная мелко-фича для **всех трёх** трекеров
...
- [ ] **2.5·27 (ГЛУБОКИЙ БАГХАНТ + внимательный анализ перед деплоем).** После ·23–·26 — тщательный разбор
  ВСЕЙ фичи целей: грубые потоки + edge + границы. Включить: grep-аудит чистоты слоёв (ADR-0030/0034/0050),
  все 3+1 родов, конкурентность записей (computed → гонок быть не должно, ADR-0052), TZ-границы дня, rollup
  смешанных единиц, фокус-лимит, mission-filter, стартер-пак. **DoD:** письменный отчёт-анализ; 0 известных
  блокеров. Только зелёный ·27 открывает FINISH.
...
- [ ] **2.5·FINISH (деплой 2.5, был `·22`).** `VERSION`-бамп → `2.5.0`, `release-2.5.0.md` + broadcast,
  ...
  **Только после зелёного ·27.** (Ранее ОТЛОЖЕН реш. Elmir 2026-06-23 — теперь это финальный шаг подэтапа,
