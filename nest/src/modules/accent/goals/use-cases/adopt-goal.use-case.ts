...
/** Use-case присвоения примера цели («Добавить себе», `POST /accent/goals/:id/adopt`, патч 9). */
@Injectable()
export class AdoptGoalUseCase {
  /**
  ...
  public async execute(id: string, accountId: string): Promise<GoalView> {
    return toGoalView(await this._goals.adopt(id, accountId));
  }
}
