...
> **📌 Статус (2026-07-06): все ДОКОВЫЕ подпункты 2.6 закрыты** — H1·1 (a/b/c: ui-ux §6, api-contracts, ADR-0057), C1 (1–5: domain-model §7 + api-contracts §7), H2·1 (a/b: ADR-0058 + domain-model §1/§5 + gamification §7 + api-contracts §5). **Осталось (ждёт сигнала «пишем код Нормисов»):** H1·2/H1·3 (фронт), C2–C5 (БД/домен/API/фронт/verify), H2·2–H2·6 (бэк/БД/API/фронт/verify). Следующая сессия при «пишем код» начинает с первого незакрытого `[ ]` выбранного трека.
