...
- **2.5·28 (РЕГРЕСС ВСЕГО АКЦЕНТА перед деплоем) — реш. Elmir (2026-06-24): не только цели, а ещё привычки
  ...
  - **Бэклог находок прогона (улучшения, НЕ блокеры ·28; решаем/планируем отдельно):**
    ...
    - [x] **H#B2-9 🟡 Ввод формы теряется при ошибке сохранения — ГОТОВО ✅ (2026-06-26, автономно).** Submit
      ...
      **Grep-аудит всех `afterClosed`-сайтов** добил пропущенный сиблинг: `goal-detail` (правка цели + «+ Подцель»
      ...
      детали проверена живьём). Остаётся **вне scope ·28** один кандидат того же класса: `avatar-crop.service` (профиль,
      фаза 1) — кроп возвращается в родителя, загрузка отдельно; при сбое загрузки кроп теряется. Флажок на будущее
      (фикс = перенос загрузки в модалку), не блокер.
  ...
  - **🧾 ХВОСТ ·28 после восстановления лимитов (НЕ блокирует FINISH — все 🟡/❓/💡, не 🔴/🟠):**
    ...
    ~26 находок зафиксировано и почти все исправлены. Открытыми осознанно оставлены **3** (H#B2-9 закрыт
    автономно 2026-06-26 — ввод формы при ошибке сохранения больше не теряется):
