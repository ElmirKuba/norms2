...
- [ ] **·12 (фронт, лента)** — рендер событий: выполнено (со временем — закрывает `completedAt`), перенесено на завтра, ожидает внимания + кнопка «В „Сегодня“», движения планки как герой экрана.
