...
### 🧩 Расширения ядра привычек ПЕРЕД 2.6 (FEAT-H1, FEAT-H2) — спроектировано 2026-07-06
> Два маленьких расширения одного ядра (привычки: лесенка + kind + карточка задачи). Каждое
> **шиппится независимо**, но проектировать логично рядом (трогают один слой). Оба — в очередь
> ПЕРЕД 2.6. Порядок нарезки — 3 шляпы (бизнес/система/человек).

**FEAT-H1 🟢 Таймер/секундомер для `timed`-привычек** _(бывш. «BUG-4» — переклассифицировано: это ФИЧА, не баг)._
...
**FEAT-H2 🟢 Полярность лесенки + `clock`-kind (сдвиг окна сна)**
