...
export interface AccountRepositoryPort {
   ...
   *
   * **Для домена это удаление, и точка** ([ADR-0068](../../../../docs/decisions/0068-deletion-belongs-to-storage.md)):
   * привычки, цели, «держусь», препятствия, микро-победы, сессии, секретные вопросы, коды —
   * всё уходит вместе с человеком. Что именно делает с этим хранилище (метка или физический
   * `DELETE`), домену знать незачем: захотим стирать по-настоящему — выключим `paranoid`, и
   * ни одна строка выше слоя 5 не изменится.
   *
   * Дерево приглашений и баны при этом остаются: это связи с другими людьми, а не его данные.
