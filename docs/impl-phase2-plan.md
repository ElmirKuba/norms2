...
- [ ] **·7 (бэк)** — публикация истории: make-цель, десять нот в порядке `1.0.0 → 2.9.0`, друг за другом с паузой (Telegram режет частоту), `broadcasted_at` проставляется. **Даты постов — сегодняшние**, версия видна в тексте и на превью.
