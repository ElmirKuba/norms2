...
- **`GET /accent/habits/:id/history?before=YYYY-MM-DD&limit=30` (2.7.3)** → история конкретной привычки для экрана «что было с этой привычкой». *(Пишется до кода; будет приведён к факту при реализации.)*
  ...
  - **Статуса `missed` в системе нет и не заводим.** `missedEstimate` считается **на лету** по текущему расписанию между `lastMarkedOn` и сегодня, без единой новой строки в БД. Это **оценка, а не факт**: расписание могли менять задним числом. Тон наружу — «последняя отметка N дней назад», не «пропущено N дней».
...
- **`updated_at` у задачи (2.7.1):** колонка ведётся с этого патча (раньше у `tasks` был только `created_at`) — для диагностики и будущих сценариев свежести. Наружу отдаётся в `TaskView`.
...
- `GET /accent/anti-habits` → `AntiHabitView[]` (активные, включая примеры) · `POST` Body `{ title, description?, targetDays?, startAt? }` → `AntiHabitView` (201; без `startAt` — старт сейчас; `startAt` в ПРОШЛОМ/сейчас → серия уже идёт (`active`) — перенос с другого приложения; `startAt` в БУДУЩЕМ → `planned` + событие `plan`). `startAt` — любой положит. unix-ms (при создании прошлое разрешено, в отличие от `/reschedule`).
- `GET /accent/anti-habits/:id` → `AntiHabitView` · `PATCH /accent/anti-habits/:id` Body `{ title?, description?, targetDays?, isActive? }` (`isActive:false` = убрать из списка) → `AntiHabitView`. **Дату старта PATCH НЕ меняет** (в прошлое нельзя, D7-пасхалка; в будущее — через `/reschedule`). Правка примера (`is_starter`) присваивает его (adoption).
...
- `GET /accent/obstacles/:id` → `ObstacleView` (контрмеры с действенностью добавятся в блоке C) · `PATCH` Body `{ name?, type?, domainKey?, trigger?, symptoms?, intensity?, isActive? }` (`isActive:false` = убрать из списка; правка примера присваивает его — adoption) · `DELETE` (каскад контрмер и журнала; ссылки из `anti_habit_events` → NULL).
