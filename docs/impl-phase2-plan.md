...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.5 «Цели» НАЧАТА:** `2.5·1` (DESIGN) ✅ — [ADR-0052](./decisions/0052-accent-goal-direction-and-computed-progress.md)
  (direction accumulate/reach/reduce + startValue; агрегаты вычисляемы → без version; forecast в едином
  пространстве доли `f`; rollup=среднее % детей; тон проективный). Следующее — **`2.5·2` (схема `goals`
  + миграция)**. Нарезка 2.5·2–·23 ниже.
