...
- [ ] **2.7·31** БД: `habits.min_version_micro_win_id?` + `goals.fallback_micro_win_id?` (FK на
  `micro_wins`, `ON DELETE SET NULL`) + миграция `00XX` → просмотр SQL → `db:migrate` на dev.
  Текстовые `minVersion`/`fallbackVersion` **остаются** (ссылка их дополняет, не заменяет).
