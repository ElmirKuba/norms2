...
### Проверка бота на localhost (появилась вместе с polling)

**Раньше было нельзя.** Вебхук требует публичного HTTPS: Telegram должен достучаться до нас, а
`localhost` для него не существует — отсюда и стоял хвост «проверять Telegram живьём негде».
**Polling переворачивает направление:** бот сам ходит к Telegram, и место запуска перестаёт иметь
значение. Dev получает полноценную приёмную заявок без всякой публичности.

⚠️ **Условие, без которого это сломает прод: нужен ОТДЕЛЬНЫЙ бот.** Апдейт Telegram отдаёт **только
одному** получателю. Если dev и прод поллят одного бота, они начнут растаскивать сообщения: часть
уедет на localhost, и человек ответа не дождётся; плюс `getUpdates` с двух процессов даёт 409.

**Как поднять (минута работы):**

1. У `@BotFather` — новый бот, как удалённый 10.08.2026 `@norms2_stage_bot`.
2. В dev-`.env`:
...
3. `make dev-restart` (не `docker restart` — см. запрет в `CLAUDE.md`).

Канал-витрину для теста тоже заводить отдельный: боевой означал бы, что отладочные посты видят
подписчики, а увиденное не удаляется вместе с постом.

**Прокси на dev обычно не нужен:** у машины разработчика есть свой доступ к Telegram. Понадобится —
те же `TELEGRAM_PROXY_URL` и `NODE_USE_ENV_PROXY`, что на проде.
