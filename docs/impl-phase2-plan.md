...
**Блок E — фронт**
- [ ] **2.7·20** Роут `/accent/obstacles` (+`:id`) в `accent.routes.ts`, вкладка в `accent.component`,
  типы в `accent.types.ts`, методы в центральном `AccentApiService` (по образцу «Держусь»),
  регистрация секции в механизме свежести (2.6.4).
...
- [ ] **2.7·22** Форма create/edit в `MatDialog` (`name`/`type`/`trigger`/`symptoms`/`intensity`),
  PII-подсказка на свободных полях, паттерн «форма сама зовёт API и закрывается при успехе».
