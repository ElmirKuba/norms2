...
account (фаза 1)
 ...
 ├─ AntiHabit           1:N   ── AntiHabitEvent 1:N (таймлайн: relapse|reschedule|plan|goal_reached)
 ├─ Obstacle            1:N   ── Counterplay 1:N (опц. → MicroWin)
 │                            └─ ObstacleEncounter 1:N (журнал столкновений, опц. → Counterplay)
...
**Task** (инстанс на день): `id`, `account_id`, `templateId?` (null = разовая задача, one-off), `goalId?`, `title`, `occurred_on` (date), `kind`, `targetValue?` (снимок currentTarget на день), `doneValue?` (сколько фактически — частичное выполнение), `status` (`pending|done|partial|skipped`), `skipReason?` (`postponed` — единственное значение; кнопки отмены задачи нет), `postponedFromTaskId?`, `priority`, `category?` (для разовых), `deadline?` (для разовых), `completedAt?`, `ladderBefore?` (снимок лесенки «до отметки» — только для отката отмены, 2.7.3; наружу не отдаётся), `created_at`, `updated_at`.
...
- **Перенос/план (D2, план, ADR-0059):** старт **идущей** серии двигается только **в будущее**. **При создании** — чекбокс «Указать своё время старта» (`datetime-local`, с секундами): прошлый момент → серия уже идёт (`active`, перенос с другого приложения, счётчик продолжается с выбранного момента — не с нуля); будущий → `plan` (старт в будущем, `planned`). Из идущей серии — «Перенести на будущее»: под CAS завершаем текущую попытку (событие `reschedule` c `heldDays`), ставим `currentAttemptStartedAt` в будущее. **Бэкфилл идущей серии в прошлое НЕ поддерживается** (правка/`reschedule` в прошлое запрещены — «создай новую с прошлым стартом, старую удали»; UI-пасхалка «машина времени», D7). Т.е. прошлое доступно ровно в точке создания.
...
## 8. Obstacle + Counterplay + ObstacleEncounter (препятствие)

> Статус: **📋 план (подфаза 2.7)** — модель спроектирована ([ADR-0062](../../decisions/0062-accent-obstacles-model.md)), кода и таблиц ещё нет. Смысл раздела: препятствие — не свойство характера («я прокрастинатор»), а **объект с именем, триггером и заранее заготовленным ответом**. В момент столкновения ресурса на изобретение решения нет, поэтому решение готовится заранее и хранится вне головы (внешняя память + самокомандование, [ADR-0049](../../decisions/0049-accent-product-principles.md)).

- **Obstacle:** `id`, `account_id`, `name`, **`type`** (обязательный, словарь), `domainKey?` (сфера жизни, мягкий ключ без FK), `trigger?`, `symptoms?`, `intensity` (1..5), `isActive`, `position` ([ADR-0054](../../decisions/0054-tracker-manual-reorder-dragdrop.md)), `isStarter` ([ADR-0051](../../decisions/0051-starter-habits-inert-showcase.md)), `version` ([ADR-0035](../../decisions/0035-concurrency-control.md)), `created_at`/`updated_at`.
- **Две оси классификации** (калька [ADR-0056](../../decisions/0056-micro-win-two-axis-taxonomy.md), как у микро-побед):
  - **`type` — природа препятствия (обязательно):** `inner_critic` (внутренний критик / стыд) · `avoidance` (избегание, прокрастинация) · `distraction` (отвлечение, залипание) · `body` (усталость, боль, недосып) · `emotion` (тревога, апатия) · `people` (люди и обязательства) · `environment` (внешние обстоятельства) · `perfectionism` («сначала идеально»). По этой оси `Recommender` (2.8) подбирает ответ **по природе проблемы**, а не по названию, которое у каждого своё.
  - **`domainKey` — сфера жизни (опционально):** общая ось со целями/привычками/микро-победами; нужна для будущей агрегации «баланс сфер» (2.11+), потребителя пока нет.
- **`intensity` (1..5) — субъективная самооценка «насколько давит»,** меняется руками. Частота столкновений — **отдельный факт** из журнала, интенсивность из неё НЕ пересчитывается: одна колонка решает одну задачу.
- **Counterplay:** `id`, `obstacle_id` (CASCADE), `text`, `linkedMicroWinId?` (SET NULL), `position`, времена. Это **свой готовый ответ**, а не совет от продукта. Привязанная микро-победа в момент столкновения **запускается таймером** ([ADR-0057](../../decisions/0057-unified-tracker-timer-component.md)) — от «что делать» до «делаю» один тап. Порядок ручной.
- **ObstacleEncounter (журнал столкновений):** `id`, `obstacle_id` (CASCADE), `occurredAt` (unix ms), `counterplayId?` (SET NULL — что применил), `outcome?` (`helped | partly | no`), `note?`. **Append-only** (`version` не нужен), единственный modify — проставление `outcome` задним числом.
- **`outcome` опционален** и спрашивается ненавязчиво вторым тапом. Пустой `outcome` = «не отмечено», а **не** «не помогло»: в знаменатель «помогало N из M» такие записи не идут (non-punitive, ADR-0049). Обязательный вопрос отвергнут — он превращает раздел в оценивание себя в худшую минуту.
- **Вычисляется на чтение** (принцип computed-агрегатов, [ADR-0052](../../decisions/0052-accent-goal-direction-and-computed-progress.md); хранимых счётчиков нет): `encountersLast30` («мешал N раз за 30 дней»), `counterplaysCount`, действенность каждой контрмеры («помогало 7 из 9»). **Сортировка контрмер остаётся ручной** — действенность лишь подсказка, список не переупорядочивается сам (человек не теряет контроль над своим набором ответов).
- **Лимиты:** мягкий фокус-лимит активных препятствий из env `ACCENT_OBSTACLE_SOFT_LIMIT` (дефолт 7) — подсказка «много фронтов сразу — может, часть в архив?», **без запрета** (как фокус-лимит целей). Жёсткие: 200 препятствий на аккаунт, 20 контрмер на препятствие — защита БД, не воспитание.
- **Связь с «Держусь»:** `anti_habit_events.obstacle_id?` (SET NULL) — рецидив может указать препятствие из списка; свободный `triggerTag` остаётся для того, чего в списке нет. На детали препятствия — «сорвался из-за этого N раз». Зависимость **строго вниз**: use-case «Держусь» зовёт `ObstacleDomainService`, обратной ссылки нет.
- **Граница с 2.8:** `Obstacle` — **нижний домен, ни о ком не знает.** Поля вроде `showInStates` НЕ заводим: иначе `obstacles` знает про состояния, а `Recommender` обязан знать про препятствия → круговая зависимость. 2.7 хранит данные и факты; `Recommender` (2.8) и дашборд (2.11) читают его сами.
- **События геймификации — НЕ эмитим в 2.7** (в отличие от «Держусь»): награждать за столкновение нельзя («чем хуже — тем больше очков»), а что здесь достижение — решает 2.9 поверх готового журнала (ADR-0062 п.13).
- **Жизненный цикл:** «убрать из списка» = `isActive:false` (история цела). `DELETE` — с каскадом контрмер и журнала; ссылки из `anti_habit_events` обнуляются (история «Держусь» не рушится).
- **ПДн:** `name`/`trigger`/`symptoms`/`note` — свободный текст → подсказка «без реальных имён/телефонов/адресов» ([ADR-0001](../../decisions/0001-data-minimization-no-pii.md), ui-ux §9). Здесь она нужнее, чем где-либо: раздел провоцирует писать про конкретных людей («начальник Иван»).
...
## 9а. Todo / TodoEvent — списки дел (2.10, блок C и D)

**Зачем сущность появилась (факт, не гипотеза).** Продукт требовал классифицировать мысль в момент
записи: чтобы записать «сходить в МФЦ», надо было решить, в какой это день и какого вида задача.
Результат измерим — **разовых задач за два с половиной месяца заведено ноль**, а живой список дел
владельца всё это время жил в стороннем заметочнике. Здесь обязателен **только заголовок**.

- **Todo:** `id`, `account_id`, `parentId?` (подзадача — та же `Todo`, рекурсия; предел глубины 3),
  `kind` (`idea | deed | purchase` — идея, дело, покупка), `title`, `note?`, `status`
  (`open | done`), `completedAt?`, `plannedOn?` (день `YYYY-MM-DD`), `waitsForEventId?` (мягкая
  ссылка на `TodoEvent`), `waitsUntil?` (день), `badge?`, `archivedAt?`, `position`.
  - **Подзадача — полноценная запись** (реш. Elmir 15.08.2026): свой статус, своя дата, своё
    ожидание. Поэтому «оплатить пошлину» и «сдать документы» могут стоять в разных днях.
  - **Выполненное не исчезает**, а уходит вниз: «что я уже сделал» — тоже информация.
  - **Покупка — отдельный вид, а не подпункт дела:** в живом списке покупки лежат сами по себе, без
    привязки к магазину, и живут иначе — их не «делают», их покупают.
- **TodoEvent:** `id`, `account_id`, `title`, `expectedOn?`, `happenedAt?` — справочник событий,
  которых ждут дела.
  - **Справочник, а не строка в записи:** одного события ждут несколько дел («приедет сварщик
    27.08» держит и перенос батареи, и переварку труб, и покупку кронштейна). Текстом в каждой
    записи — три места правки при переносе даты, и они разъедутся молча.
  - **Состоявшееся событие не удаляется, а отмечается**, и все ждавшие дела освобождаются разом.
    Удаление события тоже снимает ожидание — иначе остаётся ярлык-призрак ([ADR-0068](../../decisions/0068-deletion-belongs-to-storage.md)).
  - **Ожидание — не невыполнение.** Дело, которое ждёт мастера, не может быть сделано раньше срока;
    считать его долгом значит показывать долг там, где его нет.

## 10. Недельный слой и журналы

- ~~**WeeklyGoal** + **WeeklyGoalItem**~~ — **не заводятся** (реш. Elmir 14.08.2026, [ADR-0069](../../decisions/0069-week-is-a-horizon-not-a-list.md)). Недельный планер из продукта исключён: к моменту разбора в разделе уже было шесть мест, куда человек пишет, и седьмой список не делал бы ничего нового, зато мог быть заброшен. **Уцелела та половина замысла, что и была помечена «вычисляемая»:** недельный обзор (плотность по дням, самый плотный и самый тихий день) считается из существующих данных и показывается **блоком на дашборде**, без своей таблицы и без ввода от человека.
