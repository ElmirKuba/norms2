...
#### 🧩 2.9.1 — релизы уезжают в Telegram-канал → выйдет версией `2.9.1`

> **ADR: [`0064-telegram-release-channel.md`](./decisions/0064-telegram-release-channel.md)** —
> почему канал, а не персональные чаты; почему вебхук не нужен; почему `chat_id`, а не `@username`.
>
> **Суть (реш. Elmir, 04.08.2026):** каждый релиз официализируется — уходит постом в общий
> Telegram-канал (превью + полный текст) и становится доступен снаружи на публичной странице.
> Персональные уведомления в личку — **не сейчас**, «пока простым путём».
>
> **Особенность подфазы:** это не только разработка. Часть шагов делает **Elmir** (заводит бота и
> канал, принимает превью, жмёт кнопку публикации истории), часть — Claude. Список общий и идёт
> по порядку выполнения, а не по исполнителям.
>
> ⚠️ **Токен бота не называется в переписке** — Elmir кладёт его прямо в `.env` (dev и прод).
> Значение, произнесённое в чате, остаётся в истории навсегда.

**Подготовка**

- [ ] **E1 [Elmir]** — завести бота через `@BotFather` (`/newbot`), положить `TELEGRAM_BOT_TOKEN` в `.env`.
- [ ] **E2 [Elmir]** — создать канал, решить название и **публичный адрес** `t.me/…` (от него зависят ссылки).
- [ ] **E3 [Elmir]** — добавить бота в канал **администратором** с правом публикации.
- [ ] **E4 [Elmir]** — прописать `TELEGRAM_CHANNEL_ID=@имя_канала` в `.env`; на dev — **тестовый** канал, чтобы прогоны не летели подписчикам.

**Механика (не требует бота — можно делать параллельно с подготовкой)**
...
- [ ] **·4 (бэк+фронт)** — публичные `/releases` и `/releases/:key`: контроллер **без Guard**, только релизные ноты, **без отметок о прочтении**; экран в публичной зоне, рендер тем же `renderMarkdown` и глобальными стилями `.md`. Текст ноты и так публичен (`content/*.md` отдаётся без токена) — закрыт был только API центра уведомлений.
- [ ] **E5 [Elmir]** — приёмка страницы глазами.

**Превью**

- [ ] **E6+·4.5 [вместе]** — выбрать инструмент: MCP-дизайн (нужно подключить и авторизовать — сейчас у Claude нет ни одного рабочего) **или** собственный генератор на HTML + headless Chrome (уже стоит, единый стиль с продуктом, перерисовка всех разом одной командой) **или** Figma руками у Elmir + подстановка файлов.
- [ ] **·4.6 (скрипт)** — генератор превью 1200×630 + `make previews`.
...
- [ ] **·4.7** — отрисовать превью для всех десяти существующих нот.
...
- [ ] **·6 (бэк)** — экранирование под разметку Telegram, аккуратные переносы, обрезка.
...
- [ ] **E9 [Elmir]** — **запустить публикацию истории в боевом канале**. Кнопку жмёт владелец: пост можно удалить, но подписчики уже увидели.

**Завершение**

- [ ] **·8 (бэк+фронт)** — честная дата ноты: `published_at` вместо времени сева. Сейчас в колокольчике «2.8 вышла сегодня в 00:12», хотя это момент пересева базы.
- [ ] **·9** — `VERSION` → `2.9.1`, нота 2.9.1 (первая, что уедет в канал по-настоящему), verify, деплой на Johanna.
- [ ] **E10 [Elmir]** — финальная приёмка: канал, публичная страница, колокольчик.
...
- **Резать текст по разделам, а не по символам.** Обрыв посреди фразы — то же самое, что сломанная таблица: выглядит как небрежность продукта, а не как лимит платформы.
