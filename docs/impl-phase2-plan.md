...
- [x] **BUG-5 🟠 Перенос «→ Завтра» ДУБЛИРУЕТ ежедневную/через-день привычку — ЗАКРЫТ (`34e0f34`, найден на проде Elmir 2026-07-19).** Симптом: перенёс ежедневную задачу на завтра → назавтра в «Сегодня» она фигурирует ДВАЖДЫ (перенесённая + свежематериализованная). Репро на «через день»: перенёс дважды (на off-день, потом на due-день) → на due-дне тоже две.
  ...
  - **⚠️ Хвост:** уже задвоённые строки на проде (созданные до фикса) фикс не подчищает — при желании удалить руками/через UI (skip/delete лишних). Новые дубли не появятся.
### 🧩 2.6 — детальная нарезка: «привычки v2» + «держусь» (3 трека) — спроектировано 2026-07-06
> **📌 Статус (2026-07-06): Трек A / FEAT-H1 ГОТОВ (код+live-verify)** — H1·1 docs (ADR-0057), H1·2 код (`07c712a`), H1·3 live-verify через реальный UI (Playwright: «▶ засечь» → таймер → зачёт; микро-победы без регресса), + доработки (mode-aware, пауза, остаток+накопление, выбор продолжить/сначала, `prepSeconds`+миграция `0025`). Docs Треков C/B тоже закрыты: C1 (domain-model §7 + api-contracts §7), H2·1 (ADR-0058 + domain-model §1/§5 + gamification §7 + api-contracts §5).
...
> **Осталось (код) для 2.6/«держусь»:** Трек C — C2–C5 (БД-миграция/домен/API/фронт/verify). Начинать с первого незакрытого `[ ]`.
