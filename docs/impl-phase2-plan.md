...
- [ ] **2.4·23 (version/конкурентность)** — продумать оптимистичный лок/CAS для 2.4-записей
  (`habits`/`tasks`/лесенка): нет `version`-колонки, ладдер read-modify-write → гонка при мультидевайсе
  (ADR-0035 к accent не подключён). Долг (реш. Elmir 2026-06-22: не сейчас). **P1 «восстановление» —
  отложено post-alpha** (не бизнес-продукт, альфа).
