...
- [ ] **BUG-1 🔴 Вход на новом устройстве выкидывает другое (кросс-девайс «кик»).** Симптом: логин Chrome@iPhone → кикает Chrome@MacBook, и наоборот.
  ...
  - **Решение (ADR-уровень — согласовать с Elmir):** (а) фронт — **single-flight refresh** (дедуп параллельных refresh; вероятно, истинный фикс); (б) бэк — смягчить reuse-detect: грейс-окно на конкурентную ротацию тем же токеном / отзывать только проблемную сессию, не все (правка ADR-0046). **Сначала repro на стенде + смотреть `sessions`/`session_token_history`.** Вслепую НЕ чиним (риск сломать auth).
