...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **СЛЕДУЮЩЕЕ — `2.5·12` (подцели: rollup — прогресс родителя ВЫЧИСЛЯЕМ из детей, среднее %; глубина уже есть)
  → `2.5·13` (привычка→прогресс цели, первый кросс-домен ВНИЗ) → фронт `2.5·14–·21` → деплой/ретро `·22–·23`.**
  Нарезка ·12–·23 ниже.
