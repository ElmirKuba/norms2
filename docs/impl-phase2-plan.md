...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·13` (привычка→цель, кросс-домен ВНИЗ) — ЗАКРЫТ ✅ LIVE (смоук 9/9):** complete привычки докидывает
  ...
  ГОТОВ И LIVE-VERIFIED** (миграции до **0015**, dev чистая accounts=18). **ФРОНТ ЦЕЛЕЙ В РАБОТЕ:** `2.5·14` (типы+api) ✅, `2.5·15` (экран списка целей — заменил заглушку, закрыл
  P2.3) ✅ (prod-сборка зелёная). **СЛЕДУЮЩЕЕ — `·16` модалка создания/редактирования (title/why/direction/
  unit/target/startValue/deadline/domain/attributes/parentGoal/fallback; tone forecast) — врежется в кнопку
  «Создать» (TODO-стаб в `GoalsComponent`) → `·17` карточка цели (запись прогресса + визуализация %/forecast/
  история) → `·18` вехи → `·19` подцели → `·20` привязка привычка↔цель в форме привычки → `·21` полировка
  (вкл. фильтр по сфере) → деплой/ретро `·22–·23`.** Нарезка ·16–·23 ниже.
