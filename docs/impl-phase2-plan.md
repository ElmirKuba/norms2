...
> **Осталось (код) для 2.6/«держусь»:** Трек C — **C2 ✅ + C3 ✅ закрыты** (БД: схемы+миграция 0026+репозитории; домен+API: слайс `modules/accent/anti-habits/` подключён, live-verified 31/31 на dev); далее **C4** (фронт `/accent/anti-habits`: живой счёт-вверх/ring/«Сорвался»/история) + **C5** (финальный live-смоук). Начинать с первого незакрытого `[ ]` (C4.1).
