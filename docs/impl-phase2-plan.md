...
- **Следующее — R1: финализация файлов** (одним проходом применить к ТЕКСТУ доки):
  (а) терминология #1 (Epic Win→Goal/«Цель», PowerUp→MicroWin, BadGuy→Obstacle,
  Ally→Supporter, Secret Identity→Identity; routes `/power-ups`→`/micro-wins`,
  `/bad-guys`→`/obstacles`, `/allies`→`/supporters`); (б) merge CheckIn↔DailyMetric
  в `domain-model` (§0/§10/§12 — убрать DailyMetric/MetricRepository); (в) причесать.
