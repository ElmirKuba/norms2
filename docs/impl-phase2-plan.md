...
- [ ] **F6 (verify)** — два браузерных контекста Playwright: отметил в одном → фокус во втором → карточка обновилась; клик по стейл-карточке → значение **не** затёрто; открытая модалка откладывает обновление; накопительный таймер и «Начать сначала» не сломаны; `tsc` + prod-build.
