...
export class ManageTelegramLinkUseCase {
   ...
   *
   * **Код — единственное доказательство, что чат принадлежит админу аккаунта.** По логину
  ...
  public async linkByCode(chatId: string, codeRaw: string): Promise<void> {
    ...
    if (alreadyLinked !== null) {
      ...
      // админ узнавал об этом никогда: его заявки и уведомления просто переставали приходить.
      ...
      await this._api.sendMessage(
        ...
        [
          ...
          'чат уезжает к другому аккаунту молча, а прежний админ остаётся без уведомлений и',
          'не понимает, почему.',
          '',
