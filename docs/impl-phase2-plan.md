...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.5 «Цели» В РАБОТЕ:** `2.5·1` (DESIGN) ✅ — [ADR-0052](./decisions/0052-accent-goal-direction-and-computed-progress.md)
  ...
  (·5/·6/fix). dev-БД подчищена (throwaway удалён, accounts=18). **СЛЕДУЮЩЕЕ — `2.5·7` (схема `goal_entries` +
  миграция + репо: append-only INSERT, list cursor, агрегат-запрос) → `2.5·8` (entries domain: addEntry,
  авто-completed) → `2.5·9` (вычисляемые currentValue/%/forecast в GoalView).** Нарезка ·7–·23 ниже.
