...
| [0064](./0064-telegram-release-channel.md) | **Telegram-канал как витрина релизов** (2.9.1): общий канал, а не личные чаты; вебхук не нужен (входящих нет); порт `RELEASE_BROADCAST` + адаптер, пустой токен → логирующая заглушка; публичные `/releases` без отметок о прочтении; полный текст постом с резкой по разделам (лимиты 1024/4096); история публикуется последовательно, не задним числом; **никаких `@username`** — только `chat_id` канала из `.env` | accepted | [notifications] [phase2] [product] [152fz] |
