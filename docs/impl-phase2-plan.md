...
- **2.5·28 (РЕГРЕСС ВСЕГО АКЦЕНТА перед деплоем) — реш. Elmir (2026-06-24): не только цели, а ещё привычки
  ...
  - **Фаза B — карты регресса для Elmir (попунктно, «моё/твоё») — ГОТОВЫ ✅ (2026-06-24):**
    - [x] **·28.B1 — Цели:** [test-map-2.5-goals.md](./test-map-2.5-goals.md) (A–J + I focus-drag).
    - [x] **·28.B2 — Привычки:** [test-map-habits.md](./test-map-habits.md) (A–G: создание/задачи/лесенка/drag/пак/lifecycle).
    - [x] **·28.B3 — Микро-победы:** [test-map-micro-wins.md](./test-map-micro-wins.md) (A–F: CRUD/complete/состояния/drag/пак, акцент на M#A3-1/E2).
  - **Фаза C — совместный прогон + фикс (Elmir кликает, я смоучу/сверяю БД):**
    ...
    - [ ] **·28.C — карта «Привычки»** ([test-map-habits.md](./test-map-habits.md)) — следующая.
    - [ ] **·28.C — карта «Цели»** ([test-map-2.5-goals.md](./test-map-2.5-goals.md)).
  ...
  - **Бэклог находок прогона (улучшения, НЕ блокеры ·28; решаем/планируем отдельно):**
    > Заменяет активное ведение `triage-2.5-goals.md` — тот заморожен как **архив ·23** (история F#1–F#9),
    > новые находки фиксируем здесь. Архив: [triage-2.5-goals.md](./triage-2.5-goals.md).
    - [ ] **M#B3-1 ❓ Таксономия категорий микро-побед.** «Тип нагрузки» (11 кат.) vs «сферы жизни»
    ...
    - [ ] **M#B3-4 💡 Таймер микро-победы.** Запуск на длительность; отмена/досрочно/звук+«сделал?»/статистика
      («300с не хватило» → геймификация). Отдельная **patch-версия 2.5.x**, через 3 шляпы.
    - [ ] **M#B3-5 ❓ Серии/стрик микро-побед.** Рекомендация: **в геймификацию (2.8/2.9)** — сейчас намеренно
      «нулевое трение» (анти-выгорание, [[accent-sustainable-achievement-design]]). Решение Elmir.
    ...
    - [ ] **H#B2-9 🟡 Ввод формы теряется при ошибке сохранения (отложено, реш. Elmir).** При сбое сети/сервера
      на сохранении формы (привычки/цели/микро-победы) модалка ошибки **показывается** (X#A4-1 ✅), но форма уже
      закрыта → введённые данные пропадают. Правильный фикс — **перенести submit ВНУТРЬ модалки** (зовёт API сама,
      закрывается только при успехе; при ошибке — остаётся открытой + показывает ошибку), единообразно во всех 3
      формах Акцента. Путь редкий (только error-path), happy-path и уведомление работают → **отложено** в UX-проход
      форм (или по реальному запросу). Не блокер ·28.
  ...
  - **🧾 ХВОСТ ·28 после восстановления лимитов (НЕ блокирует FINISH — все 🟡/❓/💡, не 🔴/🟠):**
    подобрать на новом заходе. **Резюме ·28:** 3 карты ЗЕЛЁНЫЕ (цели A–J / привычки A–G / микро-победы A–F),
    ~26 находок зафиксировано и почти все исправлены. Открытыми осознанно оставлены 4:
    - [ ] **H#B2-9 🟡** ввод формы теряется при ошибке сохранения (кросс-каттинг привычки/цели/микро-победы; отложено реш. Elmir) — см. строку выше.
    - [ ] **M#B3-1 ❓** таксономия категорий микро-побед («тип нагрузки» 11 кат. vs «сферы жизни») — нужно продуктовое решение Elmir.
    - [ ] **M#B3-4 💡** таймер микро-победы (запуск/отмена/досрочно/звук+«сделал?»/статистика) — фича на будущее.
    - [ ] **M#B3-5 ❓** серии/стрик микро-побед — намеренно в геймификацию (2.8/2.9), не сейчас.
    - **Тех-долг стенда:** перед FINISH подчистить тестовый мусор в `pwdemo` (Демо A3/E1, присвоенная «Прочитать 24 книги», лишние записи/фокусы).
