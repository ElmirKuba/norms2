...
- **D3 — авто-эскалация календарной цели (ADR-0060):** [x] D3.1 ✅ util `anti-habit-goal-ladder.util.ts` — `addMonthsClamp` (зеркальное число, кламп к концу месяца) + `nextGoal(startedAt, now, manualTargetDays?)` → `{label, thresholdDays, targetDate}` (лестница 1/3/5/7/14/21/месяц/40/50/2мес/3мес/полгода/3квартала/год → +7д морковка ∞; ручная цель = пол-ступень). Чистые ф-ции; спот-верифай: 31.03→30.04, 31.01→28.02, високос 2028→29.02, перенос года ок; tsc чист. [~] D3.2 **частично**: [x] `nextGoal {label,thresholdDays,targetDate}` в `AntiHabitView` (вычисляется на чтение через util; фронт-тип синхронизирован; nest+ng tsc чисты) + util `reachedGoals` (готов, для материализации). **ОСТАЛОСЬ (след. окно):** ленивая материализация событий `goal_reached` в domain `listEvents` (первая страница): repo `latestGoalReachedThreshold(antiHabitId, sinceOccurredAt)` + port → domain вызывает `reachedGoals(started, now, targetDays, since)` и пишет события `goal_reached` (occurredAt=targetDate) идемпотентно по попытке. **Миграция НЕ нужна** (используем thresholdDays из событий, поля есть). [ ] D3.3 фронт (кольцо к `targetDate`+подпись цели+«морковка» после года; `goal_reached` в истории); [ ] D3.4 verify.
