...
- `GET /accent/todos?archived=1` → корневые записи с **вложенными подзадачами** (дерево собирает
  сервер: экран не должен делать запрос на каждую строку).
  - **Дерево отдаётся на всю разрешённую глубину — три уровня** (`MAX_DEPTH`). До 17.08.2026
    выдача обрывалась на первом: домен разрешал третий уровень, а показать его было негде —
    запись, созданная через API, не появлялась ни на одном экране. Запросов к базе столько,
    сколько уровней, а не сколько записей.
  - **`kind` необязателен и по умолчанию не фильтрует** (реш. Elmir 17.08.2026): экран показывает
    все виды одной кучей, вкладок «Дела / Идеи / Покупки» больше нет. Разделение заставляло решать,
    чем является запись, ещё до того, как она записана, — а смысл списка обратный: сгрузить из
    головы, не раскладывая. Вид остался **пометкой записи** (виден на карточке, меняется в
    деталях), но не перегородкой.
  - Параметр всё ещё принимается: `?kind=deed|idea|purchase` сузит выдачу. Неизвестное значение —
    не ошибка, а «без фильтра»: список не место для ошибок валидации.
