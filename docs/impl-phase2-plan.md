...
- [x] **2.5·23 (ретро 2.5 в 3 шляпы) — ЗАКРЫТ ✅** → [docs/retro-phase2-2.5.md](./retro-phase2-2.5.md).
  Топ-находка **P1: `fallbackVersion` введён, но не показан на карточке/детали** (anti-burnout-фича невидима —
  повтор паттерна «RPG мертвы» из 2.4) — быстрый высокоценный фикс. P2: uncomplete не откатывает прогресс
  цели (двойной счёт; решить с revoke 2.9), N+1 на rollup-списке, изоляция от дашборда (2.11). Архитектура
  чистая (grep-аудит), весь бэк live-verified, весь фронт prod-сборками.
