...
- **2.5·28 (РЕГРЕСС ВСЕГО АКЦЕНТА перед деплоем) — реш. Elmir (2026-06-24): не только цели, а ещё привычки
  ...
  - **Фаза A — мой баг-аудит (код + curl-смоуки), находки → triage-реестры:**
    ...
    - [x] **·28.A4 — Сквозное — ЗАКРЫТ ✅ (2026-06-24).** **Grep-аудит границ — чисто:** ORM-импорты
      ...
      happy-path и клиент-валидация работают, тих лишь редкий путь сервер-ошибки. Не блокирует DoD (🔴/🟠);
      кандидат на быструю полировку (заменить swallow на `_modal.error`). Блокеров нет.
  ...
  - **Фаза C — совместный прогон + фикс (СЛЕДУЮЩЕЕ, требует кликов Elmir):**
    - [ ] **·28.C** — прокликиваем 3 карты (я смоучу/проверяю БД, Elmir кликает критичное); находки → triage →
      фикс → перепроверка. **DoD ·28:** все 🔴/🟠 закрыты, карты зелёные. Только тогда — FINISH.
      Известное на входе: **X#A4-1 🟡** (тихий swallow CRUD-ошибок) — кандидат на полировку перед C или в ходе C.
