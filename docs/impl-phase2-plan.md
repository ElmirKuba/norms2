...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·13` (привычка→цель, кросс-домен ВНИЗ) — ЗАКРЫТ ✅ LIVE (смоук 9/9):** complete привычки докидывает
  ...
  ГОТОВ И LIVE-VERIFIED** (миграции до **0015**, dev чистая accounts=18). **СЛЕДУЮЩЕЕ — ФРОНТ: `2.5·14` (типы
  GoalView/EntryView/MilestoneView + GoalProgress-поля rollup/forecast + api-service) → `·15` список целей
  (заменит заглушку-вкладку) → `·16` модалка → `·17` карточка прогресса/forecast → `·18` вехи → `·19` подцели
  → `·20` привязка привычка↔цель в форме → `·21` полировка → деплой/ретро `·22–·23`.** Нарезка ·14–·23 ниже.
