...
export const telegramRequests = defineTableWithSchema<TelegramRequestFull>()(
  ...
  // Физическое удаление, а не пометка (2.9.3·35). Правило выбрано под обещание в политике:
  // «идентификатор чата не остаётся». Спрятанная строка — остаётся, и обещание стало бы
  // неправдой. Терять при этом нечего: след решения живёт в журнале действий, он переживает
  // и заявку, и аккаунт. Ссылок на эту таблицу нет, поэтому удаление никого не роняет.
  { paranoid: false },
);
