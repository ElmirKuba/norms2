...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·13` (привычка→цель, кросс-домен ВНИЗ) — ЗАКРЫТ ✅ LIVE (смоук 9/9):** complete привычки докидывает
  ...
  ГОТОВ И LIVE-VERIFIED** (миграции до **0015**, dev чистая accounts=18). **ФРОНТ ЦЕЛЕЙ В РАБОТЕ:** `2.5·14` (типы+api) ✅, `2.5·15` (список, закрыл P2.3) ✅, `2.5·16` (модалка
  создания/редактирования цели) ✅ (prod-сборка зелёная). **СЛЕДУЮЩЕЕ — `·17` карточка цели (запись прогресса
  + визуализация %/forecast/история cursor; openEdit уже готов) → `·18` вехи → `·19` подцели → `·20` привязка
  привычка↔цель в форме привычки → `·21` полировка (вкл. фильтр по сфере) → деплой/ретро `·22–·23`.** Нарезка ·17–·23 ниже.
