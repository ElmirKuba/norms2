...
- [ ] **·10 (центр уведомлений)** — попап колокольчика гаснет при открытии модалки ноты и не возвращается ([open-threads](./open-threads.md)). Чиним **не переоткрытием** (оно откинуло бы список наверх — поймал Elmir), а тем, что **не закрываем попап**, пока открыт диалог: `MatDialog` рисуется в overlay поверх, попап живёт под ним и скролл сохраняется сам. Делаем в 2.9, потому что именно она наполнила колокольчик достижениями и вехами — заходить туда станут чаще.
