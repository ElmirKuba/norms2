...
- [ ] **2.7·17** Порт+репозиторий `obstacle_encounters` (insert + keyset-список `(occurred_at, id)` desc,
  как в «Держусь») + счётчик `countSince(obstacleId, sinceMs)`.
- [ ] **2.7·18** Domain+use-cases: `POST /obstacles/:id/encounters {counterplayId?, outcome?, note?}`
  → запись + возврат обновлённого счётчика; `GET /obstacles/:id/encounters?cursor` (cursor-util);
  `PATCH /obstacles/:id/encounters/:eid {outcome}` — проставить исход позже (единственный modify в
  append-only журнале).
- [ ] **2.7·19** Вычисляемые на чтение поля (ADR-0052, хранимых счётчиков нет): `encountersLast30` и
  `counterplaysCount` в `ObstacleView`; «помогало N из M» по каждой контрмере на детали (пустой
  `outcome` в знаменатель **не** идёт — «не отмечено» ≠ «не помогло»). API-смоук: запись,
  пагинация, счётчик, отложенный `outcome`, каскад.
