...
- [ ] **F3 (бэк, защита от тихого понижения)** — `complete` с `doneValue` **меньше** уже записанного проходит только с явным `replace: true`; иначе `409 TASK_VALUE_DOWNGRADE` + актуальная задача в теле. Флаг шлёт только «Начать сначала» в таймере; накопительный сценарий (значение больше) работает как раньше. API-смоук: стейл-повтор → 409 и `doneValue` цел; накопление → 200; «начать сначала» → 200.
