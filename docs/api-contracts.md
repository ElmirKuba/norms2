...
- `GET /invites/of/:accountId` (auth) → `InviteeNode[]` = `[{ accountId, login, alias, reason, invitedAt, bannedByMe }]` — **прямые дети узла дерева** (ленивое раскрытие, F3.Д); `bannedByMe` = есть ли мой активный бан на этого участника. Доступ: `accountId` = я **или** в моём поддереве (`isAncestor`), иначе 403 (`SUBTREE_FORBIDDEN`).
  - **Удалённые потомки не скрываются, а помечаются `deleted: true`** (2.9.3·24). Раньше они выпадали из выдачи — и вместе с ними пропадала их подветка: узел держит своих приглашённых, без него они повисают в воздухе. Дерево при удалении **не перевязывается** ([ADR-0017, дополнение](./decisions/0017-account-soft-delete.md)), поэтому узел остаётся на месте, а экран показывает его приглушённым и без ссылки.
...
- `DELETE /bans/:id` (auth) → 204. **Снять вправе банивший или любой его предок по дереву**
  ([ADR-0003, дополнение](./decisions/0003-ban-semantics.md), 2.9.3·21). Посторонний получает
  **404**, а не 403: чужая запись не подтверждает даже факт своего существования.
  - Право идёт вверх, как и право банить: забанить может любой предок цели, и было бы странно,
    если бы отменить это мог только один человек из ветки.
- ✅ `POST /admin/bans` (admin) Body `{ targetId, reason }` → 204. **Причина обязательна и здесь**
  (реш. Elmir 14.08.2026): человек прочитает её при попытке входа и получит от бота — «доступ
  закрыт без объяснения» непрозрачно. Себя забанить нельзя (400). **Идемпотентно:** повтор по той
  же паре обновляет причину, второй записи не появляется (частичный unique
  `(banner, target) where active`).
  - **Ручка отдельная от пользовательской, и это не дублирование.** У `POST /bans` право идёт по
    ветке приглашений — только своё поддерево; у админа границ ветки нет. Разные правила читаются
    по адресу, а не прячутся в условии внутри метода. Человеку без роли `/admin/*` отдаёт 404,
    поэтому обойти ветку через админскую ручку нельзя.
- ✅ `DELETE /admin/bans/of/:accountId` (admin) → 204 — снимает **все** активные баны человека:
  экран «Люди и роли» смотрит на человека, а не на список чужих решений о нём.
- ✅ `POST /admin/bans/:id/lift` (admin) → 204, повтор → 404. Дверь на случай, когда снимать
  некому: банивший удалил аккаунт, ветка выше молчит или исчезла. Баны намеренно переживают
  удаление аккаунта ([ADR-0068](./decisions/0068-deletion-belongs-to-storage.md)), поэтому без
  этой двери бан становился вечным.
  - **Забанить из админки нельзя** — и не появится: бан у нас отношение внутри ветки
    приглашений, а не административная мера.
- Все пути пишут в журнал: выдача — `ban.issued`, снятие — `ban.lifted` с подробностями «кто банил, за что, снято по ветке или
  админом».

### Заявки: третий тип — просьба снять бан (2.9.3·22)

- Тип `unban` в `telegram_requests` (`check` расширен миграцией `0051`; `account_id` обязателен —
  как у `more_invites`).
- **Заводится только ботом:** в меню появился пункт «🔓 Меня забанили», спрашивается **логин** —
  один вопрос, причину не спрашиваем (её написал тот, кто банил, и решают по ней).
- ⚠️ **Ответ бота одинаков при любом исходе**: существует логин или нет, забанен или нет, создалась
  заявка или нет. Иначе бот превращается в перебиратель чужих логинов. Заявка при этом создаётся
  только когда есть кого разбанивать — админ не разгребает мусор.
- **Одобрение снимает ВСЕ активные баны** на человеке, а не выбранный: заявка про доступ, а не про
  отношения с конкретным банившим, и «сняли один из трёх» для человека выглядит как «ничего не
  изменилось». Пишется в журнал как `ban.lifted`.
- **Забаненный узнаёт о бане от бота**, если привязал Telegram: причина и что делать дальше.
  Согласие на уведомления тут не спрашивается — закрытый доступ это не рассылка. Остановил бота —
  сообщение не дойдёт, и это его выбор.

### Журнал действий

✅ **Реализовано** (таблица и запись — ·6, ручка и экран — ·14).

- `GET /admin/audit-log?limit=&action=` (admin) → `AuditEntryView[]` = `[{ id, createdAt, actorLogin: string|null, action, targetType: string|null, targetId: string|null, targetLabel: string|null, details: object|null }]`.
  Новые сверху, по умолчанию 100, потолок 500.
  - **`actorAccountId` наружу не уходит.** Экран отвечает на «кто, что, когда, над кем», и логина
    для этого достаточно; PK, отданный «за компанию», потом всплывает в URL и логах.
  - **`action` — фильтр по точному коду, незнакомый код даёт пустой список, а не 400.** Коды в
    записях остаются навсегда, а справочник `AUDIT_ACTIONS` живёт вместе с кодом: действие могут
    переименовать или убрать. Отбивать такой фильтр значило бы запрещать читать собственный журнал.
  - **Мусорный `limit` (не число, ноль, отрицательный) — это 100, запредельный — 500**, а не
    ошибка: выгрузка всего журнала одним запросом не должна получаться опечаткой в адресе.
