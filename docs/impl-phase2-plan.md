...
- [ ] **E4** — `TELEGRAM_CHANNEL_ID=@norms2` прописан на проде и в архиве. Осталось: **`TELEGRAM_OWNER_CHAT_ID`** (узнаётся после первого `/start` владельца боту) и **тестовый канал для dev** — на dev боевой канал ставить нельзя, иначе отладочные посты уедут подписчикам.
