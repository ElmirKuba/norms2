...
export interface AccountRead {
  ...
  updatedAt: string;
  /**
   * Коды ролей (`user`, `admin`) — 2.9.3·8.
   *
   * ⚠️ Нужны интерфейсу, чтобы решить, показывать ли пункт «Админка». **Правом это не является:**
   * настоящая проверка на бэке, и без роли весь `/admin` там отдаёт 404. Спрятанная кнопка
   * защитой не бывает.
   */
  roles: string[];
}
