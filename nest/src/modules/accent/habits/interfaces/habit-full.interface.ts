...
export interface HabitFull {
  ...
  /**
   * Микро-победа как **минимум на плохой день** (опц., FK на `micro_wins`, ON DELETE SET NULL).
   * Превращает текстовую подсказку `minVersion` в действие: на карточке задачи появляется кнопка
   * «Минимум сегодня: …», тап запускает таймер и закрывает задачу частично — серия жива.
   * Текст остаётся: ссылка его дополняет, а не заменяет (блок H подфазы 2.7).
   */
