...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.5 «Цели» В РАБОТЕ:** `2.5·1` (DESIGN) ✅ — [ADR-0052](./decisions/0052-accent-goal-direction-and-computed-progress.md)
  ...
  пространстве доли `f`; rollup=среднее % детей; тон проективный). **`2.5·2`/`2.5·3`/`2.5·4` — КОД ГОТОВ,
  tsc-зелёный, ЗАКОММИЧЕН** (схема goals+миграция 0012+GoalFull; порт+Drizzle-репо+GoalsModule; domain-service
  с инвариантами рода/глубины+config `ACCENT_GOAL_MAX_DEPTH`+3 ошибки). **⚠️ LIVE-VERIFY PENDING:** dev-стек
  в эту сессию собирался аномально долго (>15 мин, демон перегружен) — НЕ успел накатить 0012 и проверить boot.
  **СЛЕДУЮЩИЙ ЗАХОД (первым делом):** `make dev-up-detach` → `make db-migrate` (накат 0012) → `make db-psql`
  `\d goals` → boot-лог `GoalsModule dependencies initialized` → затем `2.5·5` (lifecycle) / `2.5·6`
  (endpoints+use-cases+DTO+view) и сквозной live-смоук всего бэка целей. Нарезка 2.5·5–·23 ниже.
...
- [~] **2.5·2 (бэк) — КОД ГОТОВ (tsc✓), LIVE-VERIFY PENDING** 🔄 — схема `goals` (+`parent_goal_id` self-FK
  cascade, +`direction`, +`start_value`; `doublePrecision`; CHECK `target<>0`; **без `version`**) +
  миграция **`0012_illegal_blur.sql`** + интерфейс `GoalFull` (`GOAL_DIRECTIONS`/`GOAL_STATUSES`/
  `GoalPausePeriod`). Коммит `feat 2.5·2`. **Pending:** накат 0012 на dev (`make db-migrate`) + `\d goals`.
- [~] **2.5·3 (бэк) — КОД ГОТОВ (tsc✓), LIVE-VERIFY PENDING** 🔄 — порт `ACCENT_GOAL_REPOSITORY`
  (`GoalCreateData`/`GoalUpdateData`/`GoalListFilters`) → Drizzle-репо (`listByAccount(status?,domain?)`/
  `listChildren`/`findOwned`/`create`/`update`; last-write-wins, без CAS) + каркас `GoalsModule` в зонтике.
  Коммит `feat 2.5·3–·4`. **Pending:** boot dev (`GoalsModule deps initialized`, DI ok).
- [~] **2.5·4 (бэк) — КОД ГОТОВ (tsc✓), LIVE-VERIFY PENDING** 🔄 — `AccentGoalDomainService`:
  ...
  `GoalNotFound` 404; `GoalPaused` 409 (заведён для ·5/·8). Коммит `feat 2.5·3–·4`. **Pending:** live-проверка
  инвариантов (нужны use-cases+API ·6 для прогона через HTTP — раньше только через boot/unit).
