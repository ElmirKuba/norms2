...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Следующее — `2.0.0·5` (фронт): пункт «Акцент» в верхнем меню `/app` + lazy-feature
  `accent` + роут `/app/accent`.** На прод НЕ деплоим до 2.0.0·6; прод-миграция 0003 — при том деплое.
