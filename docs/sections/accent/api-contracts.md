...
- `persistence` (2.9) — пара чисел постоянства для плитки: **итог** «дней с действием» (`totalDays`, не падает никогда) и **окно** `windowDays` из `windowSize` последних дней (восстанавливается само). Разбор по привычкам и достижения на дашборд не идут — они на `/accent/stats`.
...
- **Всё считается проекцией** из `tasks`/`micro_win_logs`/`goal_entries`; своя таблица одна —
