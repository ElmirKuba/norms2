...
export interface AccentSettingsFull {
  ...
  /**
   * Момент, когда аккаунту засеян персональный стартовый набор микро-побед, или null
   * (ещё не засеян). Гарантирует идемпотентность: набор создаётся один раз, повторно
   * НЕ пересоздаётся даже если пользователь удалил все микро-победы (2.2·5).
   */
  starterMicroWinsSeededAt: Date | null;
  /** Когда создано. */
  createdAt: Date;
  /** Когда изменено. */
  updatedAt: Date;
