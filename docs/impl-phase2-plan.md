...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **СЛЕДУЮЩЕЕ — `2.5·10` (схема `milestones` + миграция + репо) → `2.5·11` (milestones domain + endpoints;
  `reached` ВЫЧИСЛЯЕМ) → `2.5·12` (подцели rollup) → `2.5·13` (привычка→прогресс цели, кросс-домен вниз).**
  Нарезка ·10–·23 ниже.
