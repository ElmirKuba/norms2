...
- [ ] **2.7·32** Бэк привычек: поле в `HabitFull`/`View`/DTO, валидация ссылки через
  `MicroWinDomainService` (существует + принадлежит аккаунту; кросс-домен **вниз**), проброс в
  задачу дня (`TaskView` несёт минимум-действие, чтобы карточка знала о нём без второго запроса).
