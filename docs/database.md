...
| | `habit_tasks` | задачи дня из шаблонов привычек (до 2.10 называлась `tasks`; переименована, когда «задача» стала двусмысленной рядом с делами) | **физическое** (материализация, а не данные человека) |
| | `todos` | записи списков дел: идеи, дела, покупки; подзадачи рекурсией | мягкое (данные человека) |
| | `todo_events` | справочник событий, которых ждут дела | мягкое |
