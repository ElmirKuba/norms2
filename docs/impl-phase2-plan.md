...
- [ ] **·4 (бэк, достижения)** — правила выдачи шести: `returned` (7+ дней тишины → снова отметка), `bad_day_saved` (день с нулём задач спасён микро-победой), `ladder_raised` (первый подъём планки), `first_step`/`first_micro_win`/`first_goal`. Каждое **один раз за всё время** (защита уникальным индексом, не проверкой в коде). Выдача — **тихая строка в центр уведомлений** (`kind='personal'`, инфраструктура фазы 1, прецедент — `notifyInviteAccepted`); ни модалки, ни конфетти. Связь с notifications — через порт в фиче, чтобы «Акцент» не зависел от конкретного модуля нот.
