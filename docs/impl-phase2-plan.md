...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **`2.5·12` (rollup) — ЗАКРЫТ ✅ LIVE (смоук 13/13):** прогресс родителя = среднее % детей, computed.
  Миграции до **0015** (·12 без новой таблицы). **СЛЕДУЮЩЕЕ — `2.5·13` (привычка→прогресс цели — первый
  кросс-домен ВНИЗ: use-case привычек зовёт `AccentGoalDomainService.addEntry`; идемпотентность как у лесенки,
  один раз на переход complete; ADR-0050) → фронт `2.5·14–·21` → деплой/ретро `·22–·23`.** Нарезка ·13–·23 ниже.
