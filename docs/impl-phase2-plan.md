...
- [ ] **·2 (БД)** — `broadcasted_at` в `notifications` + миграция. Без него каждый рестарт объявлял бы все ноты заново: сидер отрабатывает при старте.
...
> **⏳ Ждут решения Elmir (без них шаги ·8/·10 не начинаю):**
> 1. **Текст заявки — где живёт для очереди?** **A:** хранить в записи, пока `pending`, стирать при решении (просто, переживает рестарт, но личные данные ложатся на диск). **B:** хранить только `message_id` сообщения в личке владельца, «Показать текст» = бот пересылает твоё же сообщение (на диск не попадает ничего, но почистишь чат — решать вслепую). *Рекомендую B.*
> 2. **Вопрос про пол — оставляем в анкете?** Он ни на что не влияет и остаётся лишним личным вопросом.
> 3. **Авто-протухание заявок** — через сколько дней `pending` закрывается сам? Иначе очередь растёт, а человек ждёт вечно. *Предлагаю 7 дней.*
