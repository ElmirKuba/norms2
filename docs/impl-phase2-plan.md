...
### Зачем это (бизнес-шляпа)
Препятствие перестаёт быть свойством характера («я прокрастинатор») и становится **объектом с
именем, триггером и заранее заготовленным ответом**. Момент столкновения — это момент, когда
ресурса на изобретение решения нет; поэтому решение готовится заранее и хранится вне головы.
Прямая реализация обоих принципов [ADR-0049](./decisions/0049-accent-product-principles.md):
**внешняя память** (не держать в голове) + **самокомандование** (решил на холодную — исполняю на
горячую). Тон — «интерес, не страх» ([ui-ux §1](./sections/accent/ui-ux.md)): препятствие изучают,
а не воюют с собой.
...
### Человеческая шляпа (что не должно сломаться)
- Список препятствий — **не список пороков**. Копирайт нейтральный: «что мешает», а не «твои
  слабости»; никаких «провалов» и стыдящих метрик.
- Главный экран препятствия — не паспорт проблемы, а **«что делать прямо сейчас»**: контрмеры
  выше описания, кнопка «Столкнулся» ведёт не к отчёту, а к списку своих ответов.
- Частота столкновений показывается как **информация для приоритета**, а не как счётчик позора
  («чаще всего мешает — значит, сюда стоит вложить контрмеру получше»).
- Интенсивность 1..5 — самооценка «на сегодня», её можно менять; это не диагноз.
...
- [ ] **2.7·2b** **Паспорт фичи** `docs/feature-obstacles.md` — заводится **до** кода, в будущем
