...
**Дока раздела готова** (синтезирована из вдохновения, источники переварены и удалены). Фаза 1 закрыта → фаза 2 начата. **Resumable-трекер фазы 2** (вычитка доки + реализация): [`docs/impl-phase2-plan.md`](./docs/impl-phase2-plan.md) — открывать его в начале каждого захода.

- [ ] **R0 — вычитка всей доки «Акцента»** (сперва идеальная дока): пройтись по всем файлам/разделам, обсудить, переписать/дополнить/выкинуть. Прогресс — в `impl-phase2-plan.md`.
...
## Фаза 4 — Раздел «Нормопост» (блог)

Открыть после Фазы 3. **TBD — обсудим и спроектируем** (суть/scope не определены): публичность, авторство/модерация, комментарии, контент без ПДн. См. [`Technical-assignment.md` §2/§6](./Technical-assignment.md).

---

## Фаза 5 — Раздел «ИнвестХелпер»

Открыть после Фазы 4. **TBD — обсудим и спроектируем** (суть/scope не определены): финансовые данные vs 152-ФЗ/no-PII, внешние API vs self-hosted. См. [`Technical-assignment.md` §2/§6](./Technical-assignment.md).

---

## Фаза 6 — Мессенджер «облачный»

Открыть после Фазы 5.

---

## Фаза 7 — Мессенджер e2e

Открыть после Фазы 6. Поднимать кодовую базу `~/coding/norms` как референс.

---

## Фаза 8 — Native (Capacitor + Electron)

Открыть после Фазы 7.
