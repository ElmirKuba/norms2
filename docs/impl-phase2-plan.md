...
> **Открытые форки (уточнить у Elmir перед ADR):** (а) D2 — старт в прошлом разрешаем? какой предел вглубь? (б) D2 — модель истории: обобщить `anti_habit_relapses` в «события» (тип-колонка) vs отдельная таблица корректировок? (в) D3 — точная лестница порогов (1/3/5/7/14/21/30/60/90/180/365?) и что показывать по достижении финала. (г) распределение D1–D6 по 2.6.1 vs 2.6.2.
