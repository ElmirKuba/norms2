...
- [ ] **·2 (БД)** — `user_achievements`: `account_id` + `code` + `awarded_at` + `metadata?`, уник `(account_id, code)`, FK cascade. Схема Drizzle + миграция + репозиторий в `database/` + порт в фиче. **Каталог достижений — константы в коде** (шесть кодов, титул, текст), не таблица: локализация не нужна, справочник на шесть строк не стоит миграции.
