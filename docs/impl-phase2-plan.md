...
> **Архитектура раздела (ADR-0050):** единый `AccentModule` + фича-папки
> `modules/accent/<feature>/` (settings/habits/goals/…); ORM в `database/`. Правило
> слоёв: контроллер→use-case всегда; кросс-домен ВНИЗ (use-case→чужой domain-service,
> не →use-case); свой порт/domain-service — по необходимости. Один модуль ⇒ нет
> межмодульных циклов (в отличие от ф.1, где их ловили в 0037/0038/0043).
