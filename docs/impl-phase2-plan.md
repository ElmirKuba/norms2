...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  Следующее — `2.4·20` (DESIGN: стартовый пак привычек — продумать и нарезать).**
...
- [ ] **2.4·20 (DESIGN: «стартовый пак привычек»)** — спроектировать аналог стартового пака
  ...
  **Открытые вопросы (решить в дизайн-ноте/ADR):**
  1. **Паритет механизма с микро-победами.** КАК у микро-побед пак хранится/принимается (seed-строки
     на аккаунт с `isStarter`? виртуальные примеры? endpoint adoption + cleanup?) — инспектировать 2.3,
     потом **зеркалить**, не изобретать. Привычкам нужен флаг-колонка примера + adopt + cleanup по образцу.
  2. **Курирование и количество.** Привычка — обязательство (тяжелее микро-победы), потому НЕ 17, а
     **3–6 тщательно подобранных**: каждая anti-burnout (низкий `minTarget`, адаптивная лесенка,
     заполнен `minVersion` «пол плохого дня» словами). Меньше = меньше паралича выбора.
  3. **Модалка** «получить стартовый пак привычек» — превью+выбор+adopt; **переиспользовать каркас
     `.dlg`** (фикс-шапка/скролл-тело/футер).

  **Предв. нарезка (финализировать в этом шаге):** ·20·1 — дизайн-нота+ADR (решить 1–3, инспекция 2.3);
  ·20·2 — бэк: seed примеров-привычек + флаг-«пример» + adopt + cleanup (зеркало микро-побед);
  ·20·3 — фронт: бейдж «пример» на карточке привычки + CTA очистки; ·20·4 — фронт: модалка пака
  (превью/adopt, каркас `.dlg`).
  ...
  **3 шляпы (кратко, развернуть в дизайн-ноте):** _БА_ — снимает паралич пустого экрана «Привычки»
  ...
  переиспользовать готовый паттерн 2.3, не плодить второй; флаг-колонка + adopt + cleanup; **без
  кросс-сущностей** (упрощено после уточнения). _Помощь людям_ — потерянному человеку примеры = опора
  «возьми готовое и подправь»; но мало и курировано (привычка ≠ микро-победа); `minVersion` словами
  уже даёт «пол плохого дня». **Вывод:** теперь это компактная история — зеркало 2.3, без
  транзакционного бандла; код — в ·20·1+.
