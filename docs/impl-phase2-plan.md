...
- **Следующее — R1: финализация + аудит консистентности** (см. раздел «R1» ниже):
  R1a применить решения к тексту (терминология/merge/состояния), R1b сквозной аудит
  согласованности 5 файлов (сущность↔эндпоинт↔экран↔таблица, единые имена, поля DTO,
  ссылки, нумерация). Self-Consistency.
...
### R1a — применить решения к ТЕКСТУ доки
- [ ] **Терминология #1** во всех файлах: имена сущностей, **routes**, экраны,
  компоненты, UI-копирайт. Epic Win→`Goal`/«Цель» · PowerUp→`MicroWin`/«микро-победа» ·
  BadGuy→`Obstacle`/«препятствие» · Ally→`Supporter`/«поддержка» · Secret Identity→
  `Identity`/«личность» · Quest→`Task`(oneOff). Routes: `/power-ups`→`/micro-wins`,
  `/bad-guys`→`/obstacles`, `/allies`→`/supporters`.
- [ ] **Merge CheckIn↔DailyMetric** в `domain-model` (§0 карта, §10, §12 — убрать
  `DailyMetric`/`MetricRepository` полностью; тренды — из CheckIn).
- [ ] **Состояния #3** — пометить в `domain-model`/`README`: `StateResolver` стартует
  упрощённым (survival/stability/growth+paused), полные 6 — позже без миграции.
- [ ] **Самокомандование** — отражено в `README §6` ✅; для дашборда — пометка к 2.10.

### R1b — аудит консистентности (сквозной, после R1a)
- [ ] **Единая терминология** — нет остатков старых имён нигде (`grep` по 5 файлам).
- [ ] **Сущность ↔ эндпоинт ↔ экран ↔ таблица** — каждая сущность покрыта, нет сирот
  (эндпоинт без сущности / экран без эндпоинта / таблица без сущности); **ноль**
  остаточных `DailyMetric`/`/metrics` во всех файлах.
- [ ] **Поля контрактов** — DTO в `api-contracts` соответствуют полям сущностей в
  `domain-model`; enum-статусы (`active|paused|…`) совпадают между файлами.
- [ ] **Ссылки** — внутренние ссылки между файлами и ADR-ссылки валидны.
- [ ] **Нумерация** — версии/подфазы/волны консистентны (нет `2.0.X` кроме каркаса, нет «волна 2.1+»).
- [ ] **Метки решений** — R6/R8/R10/R11 помечены одинаково во всех файлах.
