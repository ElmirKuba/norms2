...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  ≈2.9, «не потерять»). Следующее — `2.4·1`.**
...
- [ ] **2.4·1 (БД: habits)** — схема `habits` (title/description?/icon?/domainKey?/`attributes[]`/
  ...
  {minTarget,currentTarget,goalTarget?,step?,policy `manual|adaptive`, **+`easyStreak`/`missStreak`
  счётчики для LadderEngine §7**} — формат `ladder` (jsonb vs плоские) зафиксировать при шаге) +
  интерфейс ADR-0033 + миграция. **Получим:** таблица привычек.
...
- [ ] **2.4·5 (Habits API)** — контроллер под Guard (`GET`/`POST /accent/habits`, `GET`/`PATCH
  ...
  view. **Получим:** живое управление привычками. Verified live (CRUD, 401).
...
- [ ] **2.4·6 (RRULE-util)** — зависимость `rrule`; чистая util `isHabitDueOn(recurrence, date, tz)`
  ...
  + точечная проверка (daily / по будням / каждые N дней).
...
- [ ] **2.4·8 (ленивая материализация)** — domain `ensureTasksForDay(accountId, date, tz)`: для
  активных due-привычек без инстанса на день — создать Task-снимок (`targetValue=currentTarget`,
  pending). Идемпотентно (уник). **Получим:** задачи дня появляются из привычек при чтении.
  Verified: tsc, boot.
- [ ] **2.4·9 (выполнение: complete/uncomplete)** — domain: `complete(taskId, doneValue?)` →
  done/partial (**`partial≥minTarget` = победа**), идемпотентно, completedAt; `uncomplete` → pending.
  Инварианты (done/partial требует doneValue+completedAt). **Получим:** задачи отмечаются, в т.ч.
  частично. Verified: tsc, boot.
- [ ] **2.4·10 (перенос)** — domain `postpone(taskId)` → задача на завтра, текущая
  `skipped/postponed` (`postponedFromTaskId`). **Получим:** перенос задачи. Verified: tsc, boot.
- [ ] **2.4·11 (разовые задачи)** — domain create one-off (templateId=null, category?/deadline?) +
  списки `overdue`/`due-today`. **Получим:** разовые дела вне привычек. Verified: tsc, boot.

**Лесенка (1 шаг):**
- [ ] **2.4·12 (LadderEngine)** — domain-service по алгоритму **gamification §7** (псевдокод есть):
  на `complete` adaptive-привычки: `performed≥current` → `easyStreak++`, при `≥EASE_THRESHOLD(3)` и
  `<goalTarget` → `current+=step` (ladder.raised); `≥minTarget` → easyStreak=0 (серия цела);
  иначе → откат при 2 недоборах (ladder.lowered). Счётчики — из ·1. `manual` не трогает.
  **Recovery-ветка алгоритма зависит от `UserState` → в 2.4 БЕЗ неё, доберём в 2.8.** Очки за
  raised — TODO 2.9. Врезка в `complete` (·9). **Получим:** планка сама ползёт 1→100 без обнуления —
  ядро адаптивности. Verified: tsc, boot.

**API задач (1 шаг):**
- [ ] **2.4·13 (API задачи)** — `GET /accent/tasks?date=YYYY-MM-DD` (+ `overdue`/`due-today`),
...
**Фронт (6 шагов):**
- [ ] **2.4·14 (фронт: типы+api)** — `accent.types` (Habit/Task/Ladder/views) + методы `accent-api`
...
- **2.4 привычки + задачи + лесенка:** ✅ детализировано выше (2.4·1–·20 — мелкая де-рискованная нарезка), решения зафиксированы.
