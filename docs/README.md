...
```
                  152fz.md ───────────────┐ (ограничивает всё: что нельзя хранить)
                       │                   │
   (каждая закрытая развилка → ADR в decisions/) ──► питает доки ниже
                       │
...
5. **Решение → ADR.** Развилка, как только закрыта, фиксируется в `decisions/` и больше не «живёт в голове». (Карта развилок фазы 0 своё отработала и удалена — история в git.)
...
- `docs/impl-phaseN-plan.md` — где мы сейчас (живой трекер текущей фазы; сейчас [`impl-phase2-plan.md`](./impl-phase2-plan.md)); обзор всех фаз — [`../Technical-assignment.md` §2](../Technical-assignment.md)
- Источники вдохновения переварены в `sections/` и удалены; история — в git.

## Архив

- [`archive/phase2/`](./archive/phase2/) — завершённые подфазы фазы 2 «Акцент» с полной нарезкой
  шагов и формулировками решений. Вынесены из живого плана 17.08.2026: план распух до 4432 строк,
  и каждый заход начинался с раскопок. Ничего не удалено — только перенесено.
