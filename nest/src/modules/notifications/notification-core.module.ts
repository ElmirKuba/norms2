 ...
 * БЕЗ зависимости от `AccessControlModule`. Выделено, чтобы создавать персональные
 * уведомления могли другие области вниз (напр. `AuthModule` при регистрации по коду)
 * без втягивания контроллера/Guard и без цикла модулей (зеркало ADR-0038).
 */
@Module({
  providers: [
