...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·5** (БД+демонтаж авто-сева 2.2·5 → бэк seedPack/clear/adoption →
  API starter-pack+isStarter → фронт badge/CTA/контекстная кнопка → деплой 2.3.0). Дизайн
  согласован — детальная секция «2.3 — Стартовый набор (детально)».**
...
> **Деплоится как `2.3.0`** (шаг 2.3·5). Очки за adoption/complete — это 2.9 (геймификация), не здесь.
- [ ] **2.3·1 (БД + демонтаж авто-сева 2.2·5)** — добавить `micro_wins.is_starter` (bool, default
  false) + поле в `MicroWinFull`. **Убрать авто-сев:** удалить `ensureStarterSet` из
  `ListMicroWinsUseCase`, `claimMicroWinsStarter` (settings port/repo/domain), колонку
  `accent_settings.starter_micro_wins_seeded_at` (+ поле интерфейса + schema), импорт
  `AccentSettingsModule` из `MicroWinsModule` (больше не нужен). Миграция (add `is_starter`,
  drop старого флага). tsc зелёный, миграция на dev, boot чистый (DI ok).
- [ ] **2.3·2 (бэк)** — domain+repo микро-побед: `seedStarterPack(accountId)` (создаёт 7 из
  `STARTER_MICRO_WINS` с `is_starter=true`; опц. пропуск дублей по `title`), `clearStarters(accountId)`
  (delete `is_starter=true` владельца), adoption — `complete` и `update` снимают `is_starter`
  (стартовая → своя). `createMany` расширить флагом `is_starter`. Verified: tsc, boot.
- [ ] **2.3·3 (API)** — `POST /accent/micro-wins/starter-pack` (сев) + `DELETE …/starter-pack`
  (очистка примеров) + use-cases; `isStarter` в `MicroWinView` (mapper); adoption уже в
  domain (complete/update). **Verified live (throwaway):** пусто → POST pack → 7 с `isStarter=true`
  → complete одной → `isStarter=false` (присвоена), остальные стартовые → DELETE pack → остались
  только присвоенные → POST снова → докинулись (своё не стёрто). 401 без токена.
- [ ] **2.3·4 (фронт)** — empty-state CTA «Получить стартовый пак»; badge «пример» на `isStarter`
  + подсказка «Первое выполнение оставит победу себе»; контекстная кнопка (есть примеры →
  «Очистить примеры», нет → «Получить пак»); adoption отражается в UI (badge уходит после
  «Сделал»/«Изм.»). Signals, OnPush, чистый SCSS. Verified: tsc + prod-сборка strictTemplates.
- [ ] **2.3·5 (деплой 2.3.0)** — `VERSION`→`2.3.0`, релиз-нота `release-2.3.0.md` + запись в
  `RELEASE_NOTES` (broadcast), `prod-build`/`up` (migrate-гейт), smoke на проде (version 2.3.0,
  миграция применена, broadcast создан, экран работает).

### 2.4–2.12 — скелет (детальные шаги — при старте подфазы)
- **2.2 микро-победы:** ✅ детализировано выше (2.2·1–·7).
- **2.3 стартовый набор (отличимость + adoption):** ✅ детализировано выше (2.3·1–·5), дизайн согласован.
- **2.4 привычки:** БД `habits`(+лесенка)/`tasks` → домен+`LadderEngine`+ролловер (`@nestjs/schedule`)+API → фронт `/accent/habits` (Сегодня/Шаблоны, частичное).
