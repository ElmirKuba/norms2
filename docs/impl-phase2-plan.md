...
- [ ] **·11 (рендерер нот, ожидает решения Elmir)** — расширить `md-render.util.ts` (таблицы, цитаты `>`, инлайн-код, вложенные списки) **или** взять библиотеку. Развилка вынесена владельцу 04.08.2026: своё — ноль зависимостей и сохранённый XSS-щит (экранируем всё, потом накладываем разметку), но набор конструкций остаётся ограниченным и должен быть записан правилом; библиотека — полный CommonMark ценой `marked`(~35 КБ)/`markdown-it`(~100 КБ) **плюс обязательный `DOMPurify`**(~20 КБ), потому что готовые парсеры отдают HTML как есть.
