...
export interface AdminAccount {
  ...
  createdAt: string;
  /**
   * Есть ли активный бан (2.9.3·26).
   *
   * Нужен, чтобы не предлагать «Забанить» уже забаненному и чтобы дать снять бан там, где по
   * ветке снять некому: банивший мог удалиться, а предки — молчать.
   */
  banned: boolean;
}
...
export interface AdminAuditEntry {
  ...
  /**
   * Жив ли аккаунт, о котором запись (2.9.3·29.2). `null` — цель не аккаунт.
   *
   * Логин удалённого освобождается, поэтому одинаковый логин в старой и новой записи — не
   * обязательно один человек. Лента обязана это показывать.
   */
  targetAlive: boolean | null;
  /** Жив ли действовавший. `null` — действовала система. */
