...
- [x] **2.7·2b** ✅ 2026-07-30 — **паспорт заведён** → [`docs/feature-obstacles.md`](./feature-obstacles.md)
  ...
  **Паспорт фичи** `docs/feature-obstacles.md` — заводится **до** кода, в будущем
  ...
  - **Только для 2.7 — это эксперимент, не новая норма** (реш. Elmir 2026-07-29). На 2.8+ по
    умолчанию **не распространяем**: остальные подфазы работают как раньше (feature-file пишется в
    конце). Вопрос о распространении поднимаем отдельно, после закрытия 2.7.
...
#### 2.6.4 (номер уточняется) — свежесть данных между устройствами — ⬜ не начато, идёт ПОСЛЕ 2.7
...
> **Номер версии** тоже уточняется: `2.6.4` или `2.7.1` — решает Elmir при деплое.
>
...
- [ ] **F14 (релиз)** — `VERSION`→`2.6.4`, дописать раздел в накопительную ноту `release-2.6.4.md` (переименовать ключ), деплой + smoke.
