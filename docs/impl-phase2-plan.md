...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  `2.2·2` ✅ (бэк CRUD), `2.2·3` ✅ (API CRUD под AuthGuard) и `2.2·4` ✅ (complete=лог: идемпотентно
  по дню, TZ из Guard-аккаунта, `completedToday` реальный; событие — TODO под 2.8). Следующее —
  `2.2·5` (идемпотентный персональный стартовый набор MicroWin при первом обращении аккаунта).**
...
- [x] **2.2·4 (complete = лог)** ✅ — `POST /accent/micro-wins/:id/complete` Body `{ occurredOn? }`
  ...
  под 2.8** (event-emitter не подключён, листенера нет → публикация была бы no-op; зависимость без
...
- **Сессия 2026-06-17 (философия → продукт), решения Elmir:**
  ...
  - 📋 **Кандидат-фича «вспомню ли через неделю?»** — линза смысла при оценке цели/задачи/
    действия → **2.12+**, спроектировать в доке при подходе к этапу.
  - 📋 **Петля = заработанные награды** → принцип **геймификации (2.8)**: награда ощущается
    заработанной, не дармовой (anti-Skinner на уровне механики). Двигать версии вперёд —
    норма, если важная фича готова раньше (2.11→2.12 и т.д.).
  ...
  - 🎯 **Дизайн survival-режима (вход: 2.7 StateResolver/Recommender + 2.10 дашборд):**
    ...
    - 💬 **ОБСУДИТЬ (Claude объяснит Elmir):** что такое явный флаг `survivalFriendly` и чем он
      ...
      обоих, когда нужен явный флаг. Поднять при подходе к 2.7 или раньше по запросу.
  - 🎯 **Инвайт как заслуженное доверие (вход: инфра фазы 1 + Supporter 2.11):** «закрытая, но
