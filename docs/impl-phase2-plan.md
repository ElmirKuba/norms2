...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  Следующее — `2.4·19` (фидбэк лесенки + полировка).**
...
- [ ] **2.4·20 (DESIGN: «стартовый пак привычек» + связь с микро-победами)** — спроектировать аналог
  стартового пака микро-побед (2.3) для привычек: курируемый набор примеров-привычек с тегом
  **«пример»** (как `isStarter` у микро-побед), CTA/модалка «получить стартовый пак», adoption
  (взять пример → своя привычка), очистка непринятых примеров. Deliverable шага — **дизайн-нота +
  ADR + финальная нарезка на под-шаги ·20·1…·20·N** (код — в этих под-шагах, не здесь). 9% лимитов:
  сейчас только продумать и нарезать. Обязательно — **линза 3 шляп**.

  **Открытые вопросы (решить в дизайн-ноте/ADR):**
  1. **`minVersion` vs связанная микро-победа.** У привычки уже есть `minVersion` (текст «минимум на
     плохой день»). «Созависимая микро-победа» может это дублировать. Решить: (а) `minVersion` остаётся
     свободным текстом, а реальная микро-победа — отдельная опциональная связь; (б) «пол плохого дня»
     привычки = настоящая микро-победа, `minVersion` выводится из неё. Избежать избыточности.
  2. **Паритет механизма с микро-победами.** КАК у микро-побед стартовый пак сейчас хранится/принимается
     (seed-строки на аккаунт с `isStarter`? виртуальные примеры? endpoint adoption + cleanup?) — сперва
     инспектировать 2.3, потом **зеркалить**, не изобретать. Привычкам нужен флаг-колонка примера +
     adopt + cleanup по образцу.
  3. **Бандл привычка↔микро-победы.** Принятие примера-привычки тоже создаёт её пример-микро-победу(ы)?
     Атомарно (одна транзакция, идемпотентно)? Опциональным чекбоксом? Кросс-модуль — только ВНИЗ
     (use-case привычек → domain-service микро-побед, ADR-0050).
  4. **Курирование и количество.** Привычка — обязательство (тяжелее микро-победы), потому НЕ 17, а
     **3–6 тщательно подобранных**: каждая — anti-burnout (низкий `minTarget`, адаптивная лесенка, есть
     «пол плохого дня»). Меньше = меньше паралича выбора.
  5. **Инлайн-создание микро-победы из формы привычки** — в скоупе или отложить? Скорее **отложить**
     (кросс-модульная запись из формы усложняет контракт); старт — пак + тег + cleanup по образцу 2.3.
  6. **Модалка** «получить стартовый пак привычек» — превью+выбор+adopt; **переиспользовать каркас
     `.dlg`** (фикс-шапка/скролл-тело/футер, только что введён).

  **Предв. нарезка (финализировать в этом шаге):** ·20·1 — дизайн-нота+ADR (решить 1–6, инспекция 2.3);
  ·20·2 — бэк: seed примеров-привычек + флаг-«пример» + adopt + cleanup (зеркало микро-побед);
  ·20·3 — бэк: созависимые пример-микро-победы (если выбрано) — seed + каскад adoption;
  ·20·4 — фронт: бейдж «пример» на карточке привычки + CTA очистки; ·20·5 — фронт: модалка пака
  (превью/adopt, каркас `.dlg`); ·20·6 — (опц./отложено) инлайн-микро-победа из формы привычки.

  **3 шляпы (кратко, развернуть в дизайн-ноте):** _БА_ — снимает паралич пустого экрана «Привычки»
  («сердце продукта» → пустота особенно дорогая); пример учит хорошим дефолтам (философия). _СА_ —
  переиспользовать готовый паттерн 2.3, не плодить второй; флаг-колонка + adopt + cleanup; бандл =
  транзакция+идемпотентность. _Помощь людям_ — потерянному человеку примеры = опора «ты не один,
  возьми готовое и подправь»; но мало и курировано (привычка ≠ микро-победа); «пол плохого дня» как
  реальная микро-победа = прямо в ядро философии (0%→1%, never-miss-twice); тег+лёгкая очистка
  уважают автономию (примеры — предложение, не мусор). **Вывод:** идея сильная и в философии, но
  многошаговая → этот шаг только проектирует и режет; код — в ·20·1+.

**Деплой (1 шаг — ПОСЛЕ стартового пака, чтобы 2.4.0 включал его):**
- [ ] **2.4·21 (деплой 2.4.0)** — `VERSION`→`2.4.0`, `release-2.4.0.md` + `RELEASE_NOTES` (broadcast),
  `prod-build`/`up` (migrate-гейт), smoke. **Получим:** привычки + адаптивная лесенка + стартовый пак в проде.

**Ретро (1 шаг, можно после деплоя):**
- [ ] **2.4·22 (ретро-обзор 0.0→2.4 в 3 шляпы)** — пройтись по всему пути (фаза 1 ЛК + 2.0.0→2.4)
