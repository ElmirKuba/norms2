...
export interface TelegramRepositoryPort {
  ...
  /**
   * Удаляет закрытые заявки на вступление, поданные раньше границы (2.9.3·35).
   *
   * **Только гостевые (`join`) и только закрытые.** У заявки на вступление аккаунта нет по
   * определению — человек ещё не зарегистрирован, — поэтому её не уносит ни удаление аккаунта,
   * ни отвязка чата: без этой уборки адрес чата гостя, которому отказали, лежал бы вечно.
   * Заявки участников (`more_invites`, `unban`) сюда не попадают: они принадлежат аккаунту и
   * уходят вместе с ним.
   * @param before Граница: всё, что закрыто раньше.
   * @returns Сколько удалено.
   */
  purgeClosedGuestRequestsBefore(before: Date): Promise<number>;
