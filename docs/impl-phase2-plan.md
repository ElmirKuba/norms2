...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **ПЕРЕНУМЕРАЦИЯ (Elmir 2026-06-17):** вставлена НОВАЯ подфаза `2.3`; старые 2.3–2.11 сдвинуты
  ...
  старте (есть открытые вопросы в скелете). Затем `2.4` (привычки + лесенка + ролловер).**
...
### 2.3–2.12 — скелет (детальные шаги — при старте подфазы)
- **2.2 микро-победы:** ✅ детализировано выше (2.2·1–·7).
- **2.3 стартовый набор (отличимость + adoption) ← СЛЕДУЮЩЕЕ. ДИЗАЙН СОГЛАСОВАН (Elmir 2026-06-17):**
  - **БД:** колонка `micro_wins.is_starter` (bool, default false; миграция). **Убрать авто-сев из
    2.2·5** (флаг `accent_settings.starter_micro_wins_seeded_at` + `ensureStarterSet` в
    ListMicroWins) — пак теперь только по кнопке. Колонку флага можно дропнуть или оставить
    неиспользуемой (решить при кодинге). Существующие прод-аккаунты: их авто-засеянные победы
    станут `is_starter=false` (= «свои»), это ок.
  - **Первый заход:** пусто → дружелюбный empty-state + кнопка **«Получить стартовый пак»** (пак
    ТОЛЬКО по клику — реш. 3.3).
  - **Adoption (присвоение):** первое **«Сделал»** ИЛИ любое **«Изм.»** на стартовой → снимает
    `is_starter` (становится «своей»); остальные остаются стартовыми. Подсказка-текст «Первое
    выполнение оставит победу себе». Отдельной кнопки «оставить себе» НЕТ (реш. 3.1).
  - **Контекстный слот-кнопка (реш. 3.4):** есть стартовые → **«Очистить примеры»** (удаляет
    разом только `is_starter=true`, своё не трогает); стартовых нет → та же кнопка =
    **«Получить стартовый пак»** (до-сеивает 7 заново, реш. 3.2 — только ДОБАВЛЯЕТ, своё НЕ
    стирает; опц. пропуск дублей по названию).
  - **Фронт:** badge «пример» на `is_starter`, подсказка, контекстная кнопка, adoption по
    «Сделал»/«Изм.».
  - **Бэк/API:** `POST /accent/micro-wins/starter-pack` (сев), `DELETE …/starter-pack` (очистка
    примеров), `isStarter` в view, adoption в complete/update use-cases.
  - Суть: онбординг практикой, выбор явный, присвоение через действие («UI-UX отдача»).
  - Нарезать на шаги (·1 БД+убрать авто-сев → ·2 бэк → ·3 API → ·4 фронт → деплой) при старте.
