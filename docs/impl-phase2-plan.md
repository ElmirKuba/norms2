...
- **📋 В очереди на обсуждение (2026-08-01):** блок **`2.7.3` «деталь привычки»** — у привычек
  ...
  лежит в `tasks` и никому не показывается. Решений не принято ни одного: в блоке — 13 пунктов
  A–M, что продумать и обсудить (блокирующие — A/B/C). Номер релиза тоже под вопросом (пункт M).
...
>
> **Статус: не принято ни одного решения.** Ниже — материал к обсуждению (что продумать и что
> решить до кодинга), а **не ТЗ**. Нарезка на под-шаги — после разбора в 3 шляпы (правило нарезки
> для 2.5+) и ответов на блокирующие пункты.
...
- **M. Номер релиза — проверить перед стартом.** По правилу версионирования выше патч = багфикс
  внутри подфазы, минор = самодостаточная новая функциональность; здесь новый экран и новый
  эндпоинт, что тянет на минор. Но следующий минор **`2.8.0` уже занят дашбордом** (подфаза 2.11),
  а по смыслу это «продукт собрал данные и не вернул» — как 2.7.2, которая тоже вышла патчем.
  Решить: `2.7.3` или отдельный номер. Заодно — не выпускать ли вместе с отложенной частью 2
  патча свежести (F8–F16), раз они всё равно ждут дашборда.
