...
- [ ] **·8 (фронт, `/accent/stats`)** — полная картина: постоянство, привычки по отдельности, карточки достижений (выданные и «как получить» для остальных), «возвращался N раз». Экран, куда заходят намеренно и нечасто. Анимации — с уважением к `prefers-reduced-motion`.
