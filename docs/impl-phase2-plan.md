...
- [ ] **·8 (БД)** — таблица заявок: тип (`registration` | `more_invites`), текст от человека, `chat_id` заявителя, статус (`pending`/`approved`/`rejected`), решение и опциональная причина, timestamps. `chat_id` и текст живут **до решения** и удаляются вместе с закрытием заявки (ADR-0064 п.10).
- [ ] **·9 (бэк)** — вебхук `POST /telegram/webhook`: проверка `X-Telegram-Bot-Api-Secret-Token`, идемпотентность по `update_id`, разбор команд. **Ни одна команда владельца не выполняется без совпадения с `TELEGRAM_OWNER_CHAT_ID`.**
- [ ] **·10 (бэк)** — сценарий заявителя: `/start` → бот объясняет, что это и зачем → человек пишет заявку (с **предупреждением про личные данные**, как на «своём вопросе» восстановления, [ADR-0006](./decisions/0006-registration-field-rules.md)) → бот подтверждает приём.
- [ ] **·11 (бэк)** — сценарий владельца: заявка приходит в личку с кнопками «выдать N» / «отказать»; причина отказа — **опциональное поле**. Апрув создаёт инвайт-код и присылает его заявителю в личку; отказ — сообщает решение (с причиной, если указана).
- [ ] **·12 (бэк)** — заявки на **дополнительные приглашения** от существующих пользователей: тот же поток, но результат — увеличение `invites_remaining`, а не выдача кода.
- [ ] **E10 [Elmir]** — прогон обоих сценариев на тестовом боте: подать заявку с другого аккаунта, апрувнуть, отказать.
