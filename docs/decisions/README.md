...
Открытые вопросы «которые нужно принять» копятся в обсуждении (история развилок — в git). Как только вопрос **решён** — ответ переезжает сюда отдельным ADR со ссылкой на номер. Так решение не растворяется в переписке.
...
3. **После консолидации (✅ выполнено):** `decision-map` своё отработал и удалён (история — в git); ADR оставлены как журнал «почему так» (вне implementation-поверхности — Sonnet работает по reference-докам, не по ADR). Рациональ сохранена, чтобы не пере-решать.
...
| [0058](./0058-habit-ladder-polarity-and-clock-kind.md) | «Акцент»: полярность лесенки привычек `direction: raise\|lower` + `HabitKind='clock'` (минуты от вечернего якоря, midnight-safe) — reduce/дедлайн-семантика для привычек (кейс «сдвиг окна сна»), FEAT-H2 2.6.x; аддитивно, дефолт `raise` | accepted | [accent] [phase2] |
| [0059](./0059-anti-habit-timeline-events.md) | «Держусь» (2.6.2): единый таймлайн `anti_habit_events` (тип `relapse\|reschedule\|plan\|goal_reached`, типизированные nullable-колонки) вместо relapse-only; состояние `planned` (старт в будущем, вычисляемо); рефактор до деплоя (прод-данных нет) | accepted | [accent] [phase2] [backend] |
| [0060](./0060-anti-habit-calendar-goal-ladder.md) | «Держусь» (2.6.2): авто-эскалация цели по **календарной** лестнице (цель = ДАТА; месяц/год = зеркальное число с клампом к концу месяца; после года «морковка» +7д); вычисление на чтение; ручная цель = стартовая ступень, не потолок | accepted | [accent] [phase2] [product] |
| [0061](./0061-cross-device-data-freshness.md) | Свежесть данных между устройствами **без WebSocket**: refetch по `visibilitychange`/`focus` + посекционная ревизия `GET /accent/rev` (bump интерсептором) + `BroadcastChannel` между вкладками + optimistic `version` на `tasks` (CAS, `409` стейл-клиенту — защита от тихого затирания `doneValue`) | accepted | [architecture] [frontend] [backend] [accent] |
| [0062](./0062-accent-obstacles-model.md) | «Акцент» (2.7): препятствия — две оси (`type` обязателен + `domainKey` опц.), контрмера = свой готовый ответ (опц. микро-победа запускает таймер), append-only журнал столкновений с **опциональным** `outcome`, действенность и частота **вычисляются на чтение**, мягкий фокус-лимит через env; `Obstacle` — нижний домен, про состояния 2.8 не знает; события геймификации в 2.7 **не эмитим** (награда за столкновение = анти-Скиннер) | accepted | [accent] [phase2] [product] |

| [0063](./0063-accent-no-llm-in-critical-path.md) | «Акцент»: **ни одна функция не деградирует без ИИ** — `Recommender` 2.8 rule-based и объяснимый, обучение через частотную статистику + epsilon-greedy (без ML); LLM только на текстовых задачах и за фича-флагом; главный аргумент — приватность (тексты целей и срывов наружу не уходят), не бюджет; локальный инференс — дев-режим (на прод-VDS GPU нет) | accepted | [accent] [phase2] [product] [architecture] |
| [0064](./0064-telegram-release-channel.md) | **Telegram: витрина релизов + приёмная заявок** (2.9.1, одним списком): пост в общий канал на каждый релиз (превью + полный текст, резка по разделам, лимиты 1024/4096); публичные `/releases` без отметок о прочтении; заявки на регистрацию и на доп. приглашения — боту в личку владельцу, апрув/отказ с опциональной причиной; **вебхук нужен** (входящие есть), команды владельца только с `TELEGRAM_OWNER_CHAT_ID`; **никаких `@username`**, `chat_id` заявителя живёт до решения; история публикуется последовательно, не задним числом | accepted | [notifications] [phase2] [product] [152fz] [security] |
| [0065](./0065-release-vs-notification-split.md) | **Релиз — не уведомление:** публикация (`releases`: `key`, формат, дата выпуска, отметка о посте) отделяется от доставки (`notifications`: кому, `body`/ссылка на релиз, «прочитано»). Причина — половина колонок в общей таблице бессмысленна по построению, и `check` этого не выражает; три дефекта линии 2.9 (порядок нот, третий способ описать контент, ритуал чистки истории) — следствия одного корня. Миграция expand/contract; `release_id` с `on delete cascade` превращает чистку истории в один `delete`. Перенос текста нот из файлов в БД — **вне** этого решения | accepted | [notifications] [phase2] [architecture] [database] |
| [0066](./0066-interface-liveliness-boundaries.md) | **Где интерфейс оживает:** граница по частоте встречи, а не по красоте. Гостевая зона (лендинг, вход, приглашение) — характер и персонажи; кабинет — только отклик на действие; персонажи внутри — лишь в редких событиях (достижение, веха); в «Держусь» и «Препятствиях» мимики нет вовсе — человек приходит туда в момент срыва, и грустящий персонаж читается как осуждение | accepted | [ui-ux] [phase2] [product] |
| [0067](./0067-landing-presentation-pattern.md) | **Презентация на публичной странице: один блок, прокрутка, разные траектории** — слайдами показывается ОДИН блок, остальное лента; слайд выбирается прогрессом прокрутки через `sticky` (колесо не перехватывается); траектории входа/выхода разные на каждом слайде; на телефоне презентация остаётся, вмещается масштабом. Условия: без `requestAnimationFrame` (фоновый iframe не получает кадров), видимость по умолчанию, `prefers-reduced-motion` — отдельная ветка | accepted | [frontend] [phase2] [ui-ux] |
| [0068](./0068-deletion-belongs-to-storage.md) | **Удаление — дело хранилища, архив — состояние продукта.** Домен зовёт `delete()` и искренне считает, что удалил; слой 5 ставит `deleted_at`. Колонка во всех таблицах, флаг `paranoid` — на определении таблицы (join-safe), `paranoid: false` = физическое удаление без условий на чтении. `ON DELETE CASCADE` снимается со всех связей: база — копилка, каскад пишем свой, рекурсивный, снизу вверх, одной меткой времени. Продуктовой корзины нет: для бизнес-логики любое удаление безвозвратно | accepted | [backend] [database] [phase2] |
| [0069](./0069-week-is-a-horizon-not-a-list.md) | **Неделя — горизонт наблюдения, а не место для записей** (2.10): `weekly_goals`/`weekly_goal_items` не заводятся вовсе, недельный планер исключён из продукта. Неделя появляется одним вычисляемым блоком на дашборде (недельный %, фраза-наблюдение) — без маршрута, таблицы, намерения и ретро. Причина: за семь подфаз в продукте стало шесть мест, куда человек пишет, и седьмой список был бы первым, который не делает ничего нового; заброшенный чеклист — укор структурный, устраняемый только отсутствием сущности. Уцелевшая половина исходного замысла — та, что с самого начала помечена «вычисляемая». Состав 2.10: часовой пояс + датапикер + недельная карточка | accepted | [accent] [phase2] [product] |
| [0070](./0070-timezone-change-and-the-past.md) | **Часовой пояс: смена осознанная, прошлое не переписываем** (2.10): зона приходит от браузера и проверяется через `Intl`, а не по списку; смена — в настройках аккаунта через **два подтверждения**, во втором цифры («станет 16 августа в 02:05») и **зеркальный порядок кнопок**, чтобы промах попадал в отмену; текущий день может закрыться и добора не будет; прошлое — гибрид как есть (дни-строки не двигаются, события-моменты толкуются текущим поясом), журнал смен отвергнут: сдвинуться могут 2 записи из 72; предложение при каждом входе из чужой зоны, но строкой, с отказом, привязанным к зоне и забывающимся без единой даты в логике | accepted | [accent] [phase2] [product] [domain] |

### Ожидают решения

Все решения фазы 1 приняты (ADR-0001..0026). Отложенные на потом (⏸️) — текст Privacy Policy и ревью юриста (к деплою, фаза 1.9), backup-домен (см. [`../deployment.md`](../deployment.md) §10), опциональный email и блок будущих фаз. Карта развилок фазы 0 своё отработала — история в git.
