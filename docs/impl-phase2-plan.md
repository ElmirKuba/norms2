...
- [ ] **·2 (бэк, порт+репозиторий)** — чтение задач по шаблону: `listByTemplate(templateId, accountId, { before?, limit })`, keyset по `occurred_on desc`. Прямой запрос, без материализации.
