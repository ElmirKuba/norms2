...
> **📌 Статус (2026-07-06): Трек A / FEAT-H1 ГОТОВ (код+live-verify)** — H1·1 docs (ADR-0057), H1·2 код (`07c712a`, tsc+ng build), H1·3 live-verify через реальный UI (Playwright: «▶ засечь» → таймер → зачёт, день 100%; микро-победы без регресса). Docs Треков C/B тоже закрыты: C1 (domain-model §7 + api-contracts §7), H2·1 (ADR-0058 + domain-model §1/§5 + gamification §7 + api-contracts §5). **Осталось (код):** Трек C — C2–C5 (БД-миграция/домен/API/фронт/verify); Трек B — H2·2–H2·6 (движок/БД/API/фронт/verify). Начинать с первого незакрытого `[ ]` выбранного трека.
...
- **Нарезка (сперва дока, потом фронт; бэкенд не трогаем) — resumable подпункты:**
  ...
  - **H1·3 (verify):** ✅ 2026-07-06 — live-прогон через реальный UI (Playwright на dev-стеке):
    ...
    - [ ] **⏳ Осталось по запросу Elmir:** опциональное **время подготовки** (`prepSeconds`) у timed-привычки — как у микро-побед (поле привычки). Это full-stack (схема+миграция `0025` + DTO/интерфейс/репо/вью + инпут в форме привычки + проброс в таймер). НЕ сделано (нужна БД-миграция) — следующий шаг.
