...
- **D4 — акцент рекорд/попытка (UI):** [ ] D4.1 жирный/вес «Рекорд»+«Попытка N» в карточке списка и детали (разметка+SCSS).
- **D5 — «Рецидив» (UI):** [ ] D5.1 переименовать «Сорвался — начать заново» → «Рецидив» (кнопка/тексты/заголовки). _(D5.2 кнопка «Перенести в будущее» — идёт в D2.)_
- **D6 — drag-reorder (ADR-0054):** [ ] D6.1 БД `anti_habits.position` (+миграция/fold, бэкфилл по created_at); [ ] D6.2 бэк (repo.reorder + domain + `PUT …/reorder {ids}`, list order by position); [ ] D6.3 фронт (CDK drag + `reorderAntiHabits`, оптимистично+откат); [ ] D6.4 verify.
- **D2 — события + planned + перенос (ADR-0059; рефактор базы до деплоя):** [ ] D2.1 БД `anti_habit_relapses`→`anti_habit_events` (type + типизир. колонки + индекс; перегенерить 0026/новый 0027); [ ] D2.2 интерфейсы/порт (`AntiHabitEventFull`, типы, insert/listEvents); [ ] D2.3 домен (relapse→event; `reschedule` под CAS c heldDays; создание `startAt`→plan; `planned` в view; `INVALID_START_DATE`); [ ] D2.4 API (`/reschedule`, `/relapses`→`/events`, `AntiHabitEventView`, `state` в view, `startAt` в DTO); [ ] D2.5 фронт (история по типу события; кнопка «Перенести в будущее»+date-picker; UI `planned` = счёт-вниз до старта; чекбокс «Начать не сегодня» в форме); [ ] D2.6 verify (API+браузер).
- **D3 — авто-эскалация календарной цели (ADR-0060):** [ ] D3.1 util `addDays`/`addMonthsClamp`+генератор `nextGoal` (чистые ф-ции, спот-примеры: старт 31-го, високосный фев); [ ] D3.2 домен/view (`nextGoal {label,targetDate,thresholdDays}` на чтение TZ-aware; ленивая материализация `goal_reached` при проходе порога — как авто-complete целей); [ ] D3.3 фронт (кольцо к `targetDate`+подпись цели+«морковка» после года; `goal_reached` в истории); [ ] D3.4 verify.
- **D7 — пасхалка «машина времени» (UI):** [ ] D7.1 персонаж «?»+текст+тултип на карточке идущего «держусь» (почему нельзя в прошлое).
- **D1 — стартер-пак «держусь» ПОСЛЕДНИМ (ADR-0051 инертная витрина):** [ ] D1.1 БД `is_starter` (+миграция/fold); [ ] D1.2 бэк (`STARTER_ANTI_HABITS` под новые механики + seed/clear/adopt + endpoints; пример инертен — не считает серию до «Добавить себе»); [ ] D1.3 фронт (бейдж «пример»+CTA пак/очистить+adopt); [ ] D1.4 verify.
- **Закрытие:** [ ] feature-file `md` (красочный, под 2.6.N); [ ] финальный live-verify (браузер, все механики); [ ] бандл-деплой (VERSION-бамп + prod-migrate).
