...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **Код: ·1 ✅ (зонтик AccentModule), ·2 ✅ (таблица accent_settings, миграция 0003 на dev),
  ...
  ·5 ✅ (фронт: пункт «Акцент» + lazy-роут `/app/accent` → заглушка). **Следующее —
  `2.0.0·6` (фронт): вложенная навигация раздела (под-меню/вкладки — сиды) + заглушка
  дашборда + `AccentApiService` (settings/pause) + пауза-тумблер → затем ДЕПЛОЙ `2.0.0`.**
