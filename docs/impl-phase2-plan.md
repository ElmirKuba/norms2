...
- [ ] **2.3·5 (фронт: мотивационное описание раздела «для тяжёлых дней»)** — ui-ux-блок на экране
  микро-побед (и/или интро раздела): тёплый месседж, что раздел — для дней, когда **уже нет сил**;
  «делать мало — уже победа», «не обнуляйся, сделай хоть одно маленькое». Тон «интерес, не страх»
  (ui-ux §1), операционализация memory `accent-core-why-anti-doomscroll` + слогана ADR-0049.
  Чистый SCSS, ненавязчиво (свернуть/закрыть? — решить при шаге). Verified: tsc + prod-сборка.
