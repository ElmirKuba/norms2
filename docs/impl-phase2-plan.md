...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  `2.4·20·1` ✅ (DESIGN стартового пака привычек: инспекция 2.3 + ADR-0051 «инертная витрина», ветка Б).
  Следующее — `2.4·20·2` (бэк: `is_starter` + `STARTER_HABITS` + seed/clear + endpoints, ветка Б).**
...
- [ ] **2.4·20 (DESIGN: «стартовый пак привычек»)** — спроектировать аналог стартового пака
  ...
  **Нарезка (финал):**
  ...
  - [ ] **·20·2 (бэк)** — миграция `is_starter` на `habits`; `STARTER_HABITS`-константа (4–6 шт.);
    `seedStarterPack`/`clearStarters` в habit-domain-service (дедуп по title, только докидывает /
    удаляет непринятые); **`ensureTasksForDay` исключает `is_starter=true`** (ветка Б); adoption-флаг
    снимается при «Изм.» (update) и явном «Добавить себе»; `POST/DELETE /accent/habits/starter-pack`;
    `isStarter` в `HabitView`. Live-verify (сев → примеры НЕ в «Сегодня» → adopt → появляются).
  - [ ] **·20·3 (фронт)** — бейдж «пример» на карточке «Шаблоны»; CTA «Получить пак привычек» в пустом
    списке; контекстная кнопка «Очистить примеры» ↔ «Получить пак»; на карточке-примере кнопка
    «Добавить себе» (adopt). Verify: tsc + prod-сборка.
