...
- **2.5·26 (`maintain` — 4-й род «удерживать уровень» — combo ③) — модель «коридор+adherence» (выбор
  Elmir).** Расширение [ADR-0052](./decisions/0052-accent-goal-direction-and-computed-progress.md). 3 чекпоинта:
  ...
  - [ ] **·26.2 (фронт)** — род `maintain` в форме (подписи границ «ниж/верх»), отображение «В коридоре N%»,
    запись замера, тип `GoalDirection` += maintain, лейблы/подсказки.
  - [ ] **·26.3 (свод)** — браузер-смоук, синхронизация доки/трекера. → ·26 закрыт.
