...
- **2.5·28 (РЕГРЕСС ВСЕГО АКЦЕНТА перед деплоем) — реш. Elmir (2026-06-24): не только цели, а ещё привычки
  ...
  - **Фаза A — мой баг-аудит (код + curl-смоуки), находки → triage-реестры:**
    - [ ] **·28.A1 — Цели.** База уже в ·23-триаже (A1–A5, чисто). Догнать **новое из ·24–·27**: фокус
      (toggle/лимит/countFocused-после-lifecycle), mission-filter (tradeoff-гейт), maintain (коридор/
      adherence/edge), drag-reorder (position/focus_order), focus-drag (cross-list), границы слоёв.
    - [ ] **·28.A2 — Привычки.** Код-аудит (домен/репо/границы) + смоук: CRUD, лесенка/`ladder` (CAS/version,
      ADR-0035), задачи дня (`tasks`/recurrence/TZ), complete/uncomplete (откат прогресса цели, P2#2),
      приоритет, reorder (priority), стартер-пак (inert/adopt/clear), привычка→цель.
    - [ ] **·28.A3 — Микро-победы.** Код-аудит + смоук: CRUD, complete (дневной лог/`completedToday`),
      состояния (`disabledForStates`), energy/duration CHECK, reorder (position), стартер-пак.
    - [ ] **·28.A4 — Сквозное.** Дашборд (агрегаты/фокусные первыми), навигация, конкурентность, общий
      grep-аудит границ (ORM не течёт в домен, ADR-0030/0034/0050), тихие ошибки (F#7-класс) по разделам.
  - **Фаза B — карты регресса для Elmir (попунктно, «моё/твоё»):**
    - [ ] **·28.B1 — Цели:** [test-map-2.5-goals.md](./test-map-2.5-goals.md) (готова: A–J + I focus-drag).
    - [ ] **·28.B2 — Привычки:** новая `docs/test-map-habits.md`.
    - [ ] **·28.B3 — Микро-победы:** новая `docs/test-map-micro-wins.md`.
  - **Фаза C — совместный прогон + фикс:**
    - [ ] **·28.C** — прокликиваем карты (я смоучу/проверяю БД, Elmir кликает критичное); находки → triage →
      фикс → перепроверка. **DoD ·28:** все 🔴/🟠 закрыты, карты зелёные. Только тогда — FINISH.
