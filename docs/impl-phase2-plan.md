...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.1 ГОТОВА ЛОКАЛЬНО (·1–·4) — НЕ продим, выкатим бандлем с 2.2:
  ...
  `2.2·5` ✅ (стартовый набор 7 MicroWin при первом заходе, идемпотентно по флагу в настройках,
  миграция 0006). Следующее — `2.2·6` (фронт `/accent/micro-wins`: чипы, complete, CRUD-модалка).**
