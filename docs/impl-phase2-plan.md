...
- **2.5·27 (DRAG-TO-REORDER трекеров — UX, ДО деплоя).** [ADR-0054](./decisions/0054-tracker-manual-reorder-dragdrop.md):
  ...
  - [ ] **·27.1 — ADR + goals.position.** ADR-0054 (готов). Миграция `goals.position` (int, бэкфилл
    `row_number() over (partition by account_id order by created_at)`); схема/интерфейс/view +`position`.
  - [ ] **·27.2 — бэк goals reorder.** Порт/репо `reorder(accountId, ids)` (set position=индекс) + домен +
    use-case + `PUT /accent/goals/reorder`; листинг `order by position, created_at`. Live-смоук.
  - [ ] **·27.3 — фронт goals drag (CDK).** `DragDropModule`, `cdkDropList`/`cdkDrag` на списке целей;
    `drop` → оптимистично переставить + `reorderGoals(ids)`; откат+ошибка. Браузер-смоук.
  - [ ] **·27.4 — micro-wins + habits.** Их миграции `position` + бэк reorder + фронт CDK drag (повтор паттерна).
  - [ ] **·27.5 — верификация + свод.** Полный смоук, доку/трекер синхронизировать. → ·27 закрыт.
