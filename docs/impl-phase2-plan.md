...
- **R1 ЗАВЕРШЁН** (R1a финализация + R1b аудит). Аудит нашёл и закрыл 3 расхождения:
  ...
  **2.3 НАРЕЗАНА на шаги ·1–·6:** ·1 ✅ (БД is_starter + демонтаж авто-сева 2.2·5, миграция 0007);
  ...
  контекстная кнопка) → ·5 ui-ux мотив-описание «для тяжёлых дней» → ·6 деплой 2.3.0. **Следующее —
  `2.3·2`.** Детальная секция «2.3 — Стартовый набор (детально)».**
...
- [ ] **2.3·5 (фронт: мотивационное описание раздела «для тяжёлых дней»)** — компактный **callout
  под заголовком** микро-побед, **вместо** нынешней тонкой строки (статичный, всегда виден; не
  урок). Мягкая акцентная подложка (низкая альфа), иконка 🌱 слева, текст чуть мельче. **Текст
  (согласован, вариант «тёплый + не в ноль»):** «🌱 Раздел для тяжёлых дней. Нет сил на большое —
  сделай одно крошечное. Смысл не в том, чтобы успеть больше, а чтобы не упасть в ноль.» Тон
  «интерес, не страх» (ui-ux §1); операционализация memory `accent-core-why-anti-doomscroll` +
  слогана ADR-0049. Чистый SCSS, OnPush. Verified: tsc + prod-сборка strictTemplates.
- [ ] **2.3·6 (деплой 2.3.0)** — `VERSION`→`2.3.0`, релиз-нота `release-2.3.0.md` + запись в
