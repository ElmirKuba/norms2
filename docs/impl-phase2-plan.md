...
- **Нарезка (сперва дока, потом фронт; бэкенд не трогаем):**
  - [ ] **H1·1 (docs):** `domain-model`/`ui-ux`/`api-contracts` accent — доступность таймера для `timed`-привычек, источник длительности, семантика завершения; мини-ADR «единый таймер-компонент для трекеров».
  - [ ] **H1·2 (frontend):** обобщить таймер-модалку → общий компонент; «▶ засечь» на карточке `timed`-задачи; провод завершения → существующий complete(performed).
  - [ ] **H1·3 (verify):** live-смоук — создать `timed`-привычку, прогнать таймер, финиш → задача done, лесенка сместилась.
...
- **Нарезка:**
  - [ ] **H2·1 (docs+ADR):** ADR полярности лесенки + `clock`-kind; правки `domain-model`/`gamification §7`/`api-contracts §5`.
  - [ ] **H2·2 (backend):** зеркальная ветка `LadderEngine` (`direction='lower'`) + `clock`-kind + утилита минут/переход через полночь + инварианты.
  - [ ] **H2·3 (DB):** миграция — `ladder.direction` дефолт `raise` + опц. `anchorMinutes` (jsonb, без слома).
  - [ ] **H2·4 (API/DTO):** zod-DTO (`kind`, `ladder.direction`, якорь) + синхрон контрактов.
  - [ ] **H2·5 (frontend):** тайм-пикер целевого времени; формат минуты→HH:MM; инвертированный фидбэк.
  - [ ] **H2·6 (verify):** live-смоук — `clock`-привычка «отбой ≤ 3:00», ввод факта, лесенка сдвигает цель вниз через полночь.
...
- **Нарезка (B/S/A/verify):**
  - [ ] **2.6·C1 (docs):** досверить `domain-model §7`/`api-contracts §7`/`ui-ux` держусь (по факту почти готово) — зафиксировать concurrency рецидива (CAS), семантику `ALREADY_RELAPSED`, событие-хуки для 2.9, PII-границы.
  - [ ] **2.6·C2 (DB/B):** аддитивная миграция `anti_habits` + `anti_habit_relapses` (PK `uuidv7___unixmillis`, FK `ON DELETE`, `version` где нужно) + репозитории.
  - [ ] **2.6·C3 (domain+API/S+A):** `AntiHabitDomainService` (create/patch/relapse + обновление рекорда + вычисляемая серия) + контроллер `/accent/anti-habits` + zod-DTO + гард `ALREADY_RELAPSED` + рецидив под version/CAS + эмит `anti_habit.held/relapsed` (без начисления).
  - [ ] **2.6·C4 (frontend):** `/accent/anti-habits` список + деталь: **живой счёт-вверх** дн:ч:мин:сек (`setInterval`), circular-ring к `targetDays`, кнопка «Сорвался» (подтверждение + поле триггера/`note`), история попыток (cursor); тон non-punitive.
  - [ ] **2.6·C5 (verify):** live-смоук — создать держусь, таймер идёт вверх, срыв сбрасывает и пишет попытку, **рекорд сохраняется**, история и `ALREADY_RELAPSED` работают.
